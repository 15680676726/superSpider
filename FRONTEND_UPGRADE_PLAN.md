...
### 0.1 `2026-04-06` Buddy-first 补充口径

- 默认首入口已收口为 `buddy-onboarding`，用于一次性完成人类身份档案、方向澄清问答、长中短期规划起点，以及 Buddy 绑定。
- `/chat` 是 Buddy 主场。Buddy 是主脑唯一对外人格壳，人类默认只需要在这里看到“最后目标 + 当前任务 + 单一步动作”。
- `Industry` 不再承担第二套身份创建；它只负责查看、校准、调整已经生成的执行载体与执行位，不允许再长回第二套 onboarding / bootstrap。
- `Runtime Center` 不是 Buddy 主场；它负责展示运行态、治理态、证据态和主脑摘要，真正的人机陪跑仍应回到 `Chat`。
...
- 前端所有 Buddy 数据都必须从正式 surface / profile / projection 读取，不允许再回到本地缓存拼真相。
- 聊天页里的 Buddy 摘要、属性、成长和进化展示，必须围绕“伙伴”心智，而不是把 Buddy 渲染成一个普通 seat。
- 任何“给人类派任务”的动作，都应优先通过聊天语境表达，而不是再长出独立的旧式任务派发表单。
