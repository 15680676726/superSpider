...
- **Console** — Use the [Console](./console) under **Run → Capability Market → Skills**
...
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| **cron**                     | Scheduled jobs. Create, list, pause, resume, or delete jobs via `copaw cron` or Console **Run → Runtime Center → Automation**; run on a schedule and send results to a channel. | Built-in                                                       |
...
In the [Console](./console), go to **Run → Capability Market → Skills** to:
...
Console under **Run → Runtime Center → Automation**; no need to edit skill files.
...
The Console currently imports skills from these SkillHub sources by default:

- `https://skillhub-1388575217.cos.ap-guangzhou.myqcloud.com/skills/...zip`
...
- If import fails, first check URL completeness, supported domains, and outbound network access.
