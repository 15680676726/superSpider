...
**Status:** 已完成。Task 4 的组合命令在本地超时过一次，最终按分组和整文件回归完成验收。
...
- `src/copaw/kernel/query_execution_prompt.py`
  - 把 activation path hints 暴露进 query execution prompt appendix
...
- [x] **Step 1: 写失败测试，覆盖 planning 被路径改变**
...
- [x] **Step 3: 最小实现 planner path consumption**
...
- [x] **Step 1: 写失败测试，覆盖 execution path guidance**
...
- [x] **Step 3: 最小实现 execution 接线**

实现内容：
- `compiler.py` 把 path guidance 暴露进 compiled payload / task seed
- `query_execution_prompt.py` 在 activation context 下输出 dependency / blocker / recovery / contradiction path hints
