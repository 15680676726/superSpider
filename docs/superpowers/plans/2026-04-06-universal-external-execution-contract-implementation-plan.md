...
**Goal:** Add one universal external-source execution base that injects formal provider truth, enforces bounded execution, and resolves host compatibility before CoPaw claims that an external external-source is formally usable.

**Architecture:** Reuse the existing external-source/package/candidate/trial/lifecycle/capability spine and extend the current `adapter/runtime` seams instead of writing external-source-specific fixes. The new work is split into three platform contracts: provider injection, execution envelope, and host compatibility, plus the probe/promotion/evidence read model needed to keep those contracts operator-visible and off the chat hot path.
...
- No external-source-specific `if provider == X` branches may enter the core path.
- No external-source may treat host config scraping as the primary formal execution path.
- No adapter or runtime external-source action may default to unbounded wait.
...
- `src/copaw/capabilities/project_donor_contracts.py`
  - Extend installed-external-source metadata with provider injection requirements, execution envelope defaults, and compatibility hints.
...
- `src/copaw/app/routers/capability_market.py`
  - Wire install/probe/promotion flow to the new contracts while keeping external-source expansion on cold paths only.
...
- `src/copaw/state/donor_trust_service.py`
  - Track timeout/auth/compatibility failures as external-source trust signals.
...
- `src/copaw/capabilities/donor_provider_injection.py`
  - Resolve external-source-facing provider contracts from CoPaw formal provider truth and build transport-safe injection payloads.
- `src/copaw/capabilities/donor_execution_envelope.py`
  - Own timeouts, heartbeat, cancellation, kill, retry, and normalized error taxonomy for external-source actions.
- `src/copaw/capabilities/donor_host_compatibility.py`
  - Evaluate external-source/host compatibility and produce `compatible_native / compatible_via_bridge / blocked_*` results.
...
### Task 1: Add formal external-source execution contract fields
...
- external-source package metadata to expose `provider_injection_mode`
- external-source package metadata to expose `execution_envelope`
- external-source package metadata to expose `host_compatibility_requirements`
...
Expected: FAIL because formal external-source execution contract fields do not exist yet.
...
git commit -m "feat: add formal external-source execution contract fields"
```

### Task 2: Implement external provider injection contract
...
Cover:
- provider truth resolves from CoPaw provider facade, not external-source-local guessing
...
- runtime-provider facade adapter for external-source use
- external adapter execution path that injects provider truth before invoking external-source actions
...
git commit -m "feat: add provider injection contract"
```

### Task 3: Implement universal external-source execution envelope
...
Expected: FAIL because external-source actions still bypass a shared execution envelope.
...
git commit -m "feat: add external-source execution envelope"
```

### Task 4: Implement external-source host compatibility contract
...
Cover:
- external-source compatibility can return `compatible_native`
- generic alias bridge can return `compatible_via_bridge`
- missing runtime/provider requirements return typed `blocked_*`
- external-source trust updates record compatibility failures without project-specific flags
...
git commit -m "feat: add external-source host compatibility contract"
```
...
Cover:
- runtime external-source can become `runtime_operable` after start/probe/stop proof
- adapter external-source can become `adapter_probe_passed` only after a minimal business action succeeds
...
git commit -m "feat: add external-source probe and verified stage boundary"
```
...
Cover:
- Runtime Center external-source read model shows provider resolution result
- Runtime Center external-source read model shows compatibility result
- Runtime Center external-source read model shows timeout/cancel/probe outcomes
- evidence attribution includes normalized error taxonomy and selected external-source action
...
git commit -m "feat: expose external-source execution contract truth"
```

### Task 7: Run adjacency regression and verify hot-path isolation

**Files:**
- Modify: `docs/superpowers/specs/2026-04-06-universal-external-execution-contract-design.md`
...
Expected: PASS and no new chat/runtime preflight dependency on external-source discovery or external-source install paths.
...
- what still remains bounded by external-source contract assumptions
- that external-source expansion remains a cold path, not a chat hot path

- [ ] **Step 4: Commit**

```bash
git add docs/superpowers/specs/2026-04-06-universal-external-execution-contract-design.md TASK_STATUS.md
git commit -m "docs: record verified external-source execution contract state"
```
...
- Keep all external-source discovery, install, probe, and compatibility work off the main brain chat hot path.
- If a external-source needs a bridge, the bridge must be generic enough to plausibly help other donors.
- If a external-source still requires project-specific logic after Tasks 1-6, stop and document the remaining incompatibility instead of hiding it in the core path.
