...
**Tech Stack:** FastAPI backend, console frontend, Runtime Center, Industry, Chat, capability market, external capability/runtime subsystem.
...
| --- | --- | --- | --- |
| Track D: Capability Install -> Real Use | Formal capability install/use within the currently supported external-source intake contract. This includes market-backed skill path, GitHub Python project package under the current `Python + GitHub + isolated-venv` contract, and formal runtime/service provider under the current runtime contract. Current live-verified examples include `project:black` and `runtime:openspace`. | Capability Market, Runtime Center capability surface, target seat/effective capability surface, real execution result surface | Arbitrary external source, arbitrary language, arbitrary build system, arbitrary repository layout, arbitrary protocol, universal typed action discovery from any random external source |
...
- "we think other external sources like it will probably work"
