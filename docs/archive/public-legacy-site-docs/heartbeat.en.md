...
digests, scheduled reminders” — Spider Mesh runs without you sending a
message.
...
1. You have a file **HEARTBEAT.md** (by default under the working dir
   `~/.copaw/`). Its content is **what to ask Spider Mesh each time** (one
   block of text; Spider Mesh sees it as one user message).
2. The system runs on your **interval** (e.g. every 30 minutes): read
   HEARTBEAT.md → send that as the user message → Spider Mesh replies.
3. **Whether the reply is sent to a channel** is controlled by **target** in
   config:
   - **main** — Run Spider Mesh only; do not send the reply anywhere (e.g. for
     local check-ins or logs).
   - **last** — Send the Spider Mesh’s reply to **the channel/session where you
     last talked to Spider Mesh** (e.g. if you last used DingTalk, the
