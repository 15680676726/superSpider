...
**控制台** 是 Spider Mesh 内置的 Web 管理界面。运行 `copaw app` 后，在浏览器中打开
...
- 查看和管理所有聊天会话
- 管理定时任务
- 编辑 Spider Mesh 的人设和行为文件
- 开关技能以扩展 Spider Mesh 的能力
- 管理MCP客户端
- 修改运行配置
- 配置 LLM 提供商并选择使用的模型
- 管理工具所需的环境变量

左侧侧边栏按当前产品结构分为 **对话**、**运行**、**系统设置** 三组，其中 **运行**
下还包含“洞察与优化”等子分组。下面按顺序逐一介绍每个功能的操作方法。
...
这是你和 Spider Mesh 对话的地方。需要直接进入聊天时，可从侧边栏打开这个页面。

聊天 (screenshot removed)

**发送消息：**
在底部输入框中输入内容，按 **Enter** 或点击发送按钮（↑），Spider Mesh 会实时回复。
...
在这里创建和管理 Spider Mesh 按时间自动执行的定时任务。

定时任务 (screenshot removed)

**创建新任务：**

> 如果定时任务没有创建成功，可以参考 [FAQ](./faq) 的 **定时任务错误排查** 寻找原因

创建定时任务的 **最简单的方式是直接与 Spider Mesh 对话**，让他为你创建。例如你想在钉钉上收到喝水提醒，则在钉钉上与 Spider Mesh 对话：“帮我创建一个定时任务，每隔 5 分钟提醒我喝水。”创建完成后，可以在控制台的 **运行 → 运行中心 → 自动化** 页面看到创建好的定时任务。

另外一种方式是在控制台页面创建：

1. 点击 **+ 创建任务** 按钮。

   创建定时任务 (screenshot removed)

2. 按区域填写表单：
   - **基本信息** —— 给任务一个 ID（如 `job-001`）、一个名称（如「每日摘要」），
     并打开启用开关。
   - **调度** —— 填写 Cron 表达式（如 `0 9 * * *` = 每天上午 9 点）并选择时区。
   - **任务类型及内容** —— 选择 **文本**（发送固定消息）或 **Agent**（向 Spider Mesh 提问并
     转发回复），然后填入具体内容。
...
在这里编辑定义 Spider Mesh 人设和行为的文件——SOUL.md、AGENTS.md、
HEARTBEAT.md 等——全部在浏览器中完成。

工作区 (screenshot removed)
...
在这里管理扩展 Spider Mesh 能力的技能（如读取 PDF、创建 Word 文档、获取新闻等）。

技能 (screenshot removed)
...
在这里配置 LLM 提供商并选择 Spider Mesh 使用的模型。Spider Mesh 同时支持云提供商（需要
API Key）和本地提供商（无需 API Key）。

模型 (screenshot removed)
...
**查看和删除模型：**
已下载的模型列在管理面板中，显示文件大小、来源标记和删除按钮。
...
- 模型来自 Ollama 守护进程（不由 Spider Mesh 直接下载）
- 模型列表与 Ollama 自动同步
...
> **注意：** 云提供商 API Key 的有效性需要用户自行保证，Spider Mesh 不会验证。
>
...
在这里管理 Spider Mesh 的工具和技能在运行时需要的环境变量（如 `TAVILY_API_KEY`）。

环境变量 (screenshot removed)
...
> **注意：** 环境变量值的有效性需要用户自行保证，Spider Mesh 只负责存储和加载。
>
...
| 页面     | 侧边栏路径        | 你能做什么                                         |
| -------- | ----------------- | -------------------------------------------------- |
| 聊天     | 对话 → 对话       | 和 Spider Mesh 对话、管理会话                            |
| 频道     | 系统设置 → 频道   | 启用/禁用频道、填入凭据                            |
| 会话     | 对话 → 对话       | 筛选、重命名、删除会话                             |
| 定时任务 | 运行 → 运行中心 → 自动化 | 创建/编辑/删除任务、立即执行                 |
| 工作区   | 运行 → 智能体工作台 → 工作区 | 编辑人设文件、查看记忆、上传/下载            |
| 技能     | 运行 → 能力市场 → 技能 | 启用/禁用/创建/删除技能                        |
| MCP      | 运行 → 能力市场 → MCP  | 启用/禁用/创建/删除MCP                         |
| 运行配置 | 系统设置 → 智能体配置 | 修改运行配置                                 |
| 模型     | 系统设置 → 模型   | 配置提供商 API Key、管理本地/Ollama 模型、选择模型 |
| 环境变量 | 系统设置 → 环境   | 添加/编辑/删除环境变量                             |
