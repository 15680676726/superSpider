...
1. 你有一个文件 **HEARTBEAT.md**（默认在工作目录 `~/.copaw/` 下），里面写的是**每次心跳要问 Spider Mesh 的内容**（一段或几段话都行，Spider Mesh 会当成一条用户消息）。
2. 系统按你配置的**间隔**（例如每 30 分钟）执行一次：读取 HEARTBEAT.md → 用这段内容去问 Spider Mesh → Spider Mesh 回复。
3. **发不发到频道** 由配置里的 **target** 决定：
   - **main**：只跑 Spider Mesh，不把回复发到任何频道（适合只做「自检」、结果自己看日志或别处）。
   - **last**：把 Spider Mesh 的回复发到你**上次和 Spider Mesh 对话的那个频道/会话**（例如上次你在钉钉和它聊，这次心跳的回复就发到钉钉）。
...
文件路径默认是 `~/.copaw/HEARTBEAT.md`。内容就是「每次要问 Spider Mesh 什么」，纯文本或 Markdown 都行，Spider Mesh 会整体当作一条用户消息。
...
示例（只跑 Spider Mesh、不发到频道，每 30 分钟）：
