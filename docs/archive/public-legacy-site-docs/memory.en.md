...
> The memory design draws on open-source practices and is implemented with [ReMe](https://github.com/agentscope-ai/ReMe).
...
| Vector Search (embedding credentials resolved explicitly or via inheritance) | Full-text Search (`FTS_ENABLED=true`) |                      Actual Search Mode                       |
| :-----------------------------------------------------------------------: | :-----------------------------------: | :-----------------------------------------------------------: |
...
> **Recommended**: Keep `EMBEDDING_FOLLOW_ACTIVE_PROVIDER=true` and `FTS_ENABLED=true`. If your active provider is already a supported remote provider, memory can use hybrid Vector + BM25 retrieval without separate embedding configuration.
