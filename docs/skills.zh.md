...
- **控制台** — 在 [控制台](./console) 的 **运行 → 能力市场 → 技能** 页面操作
- **工作目录** — 按本文步骤直接编辑文件
...
| Skill 名称                   | 说明                                                                                                                                        | 来源                                                           |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| **cron**                     | 定时任务管理。通过 `copaw cron` 或控制台 **运行 → 运行中心 → 自动化** 创建、查询、暂停、恢复、删除定时任务，按时间表执行并把结果发到频道。 | 自建                                                           |
...
在 [控制台](./console) 侧栏进入 **运行 → 能力市场 → 技能**，可以：
...
首次运行时会从包里把 **Cron** 同步到 `~/.copaw/active_skills/cron/`。它提供「按时间表执行任务并把结果发到频道」的能力；具体任务的增删改查用 [CLI](./cli) 的 `copaw cron` 或控制台 **运行 → 运行中心 → 自动化** 完成，不需要手写 cron 以外的配置。
...
当前控制台默认支持导入以下 SkillHub 来源的 Skills：
