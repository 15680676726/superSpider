...
> 记忆设计参考开源实践，并由 [ReMe](https://github.com/agentscope-ai/ReMe) 实现。
...
| 向量检索可用（显式配置或已继承到 embedding 凭据） | 全文检索（`FTS_ENABLED=true`） |               实际检索模式               |
| :------------------------------------: | :----------------------------: | :--------------------------------------: |
...
> **推荐**：保持 `EMBEDDING_FOLLOW_ACTIVE_PROVIDER=true` 与 `FTS_ENABLED=true`。如果当前已配置的是受支持的远程模型提供方，通常无需再单独填写 embedding 配置，也能直接启用向量 + BM25 混合检索。
