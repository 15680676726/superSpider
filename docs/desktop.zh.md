...
本文档说明如何在 Windows 和 macOS 系统上安装和使用 Spider Mesh 桌面应用。
...
1. **下载安装包**
   从 [Release 页面][releases]下载 `Spider-Mesh-Setup-<version>.exe` 文件

2. **运行安装程序**
   双击 `.exe` 文件，按照安装向导提示完成安装
   - 默认安装位置：`C:\Users\<你的用户名>\AppData\Local\SpiderMesh`
...
- **特点**: 静默启动，无终端窗口，界面简洁
- **适用场景**: 正常使用，不需要查看技术日志
- **启动方式**: 双击桌面或开始菜单中的 “Spider Mesh” 图标
...
- **适用场景**:
  ...
  - 开发测试
  - 报告 Bug 时需要提供日志
- **启动方式**: 双击开始菜单中的 “Spider Mesh（调试）” 图标
- **日志内容**:
  - 应用启动信息
  - Python 错误堆栈
  - API 调用日志
...
**Q: 应用启动后没有反应？**
A: 使用 “Spider Mesh（调试）” 启动，查看终端输出的错误信息

**Q: 如何卸载？**
A: 在 Windows 设置 → 应用 → 已安装的应用 中找到 “Spider Mesh” 后卸载
...
1. **下载压缩包**
   从 [Release 页面][releases]下载 `Spider-Mesh-<version>-macOS.zip` 文件

2. **解压缩**
   双击 `.zip` 文件自动解压，得到 `Spider Mesh.app` 应用

3. **移动到应用程序文件夹 (可选)**
   将 `Spider Mesh.app` 拖到 `/Applications` 文件夹

### 首次启动：解除系统安全限制

#### 为什么需要手动信任？

当前 Spider Mesh 桌面构建**未经过 Apple 开发者签名和公证（Notarization）**，macOS Gatekeeper 会默认阻止运行。
...
1. **右键点击**（或 Control + 点击）`Spider Mesh.app`
2. 在菜单中选择 **"打开"**
...
```bash
# 切换到应用目录
cd /Applications  # 或您的 Spider Mesh.app 所在目录
...
**Q: 如何卸载？**
A: 将 `Spider Mesh.app` 拖到废纸篓，然后删除 `~/.copaw` 配置文件夹
