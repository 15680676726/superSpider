...
> 💡 **After install & start**: Before configuring channels, you can open the [Console](./console) (`http://127.0.0.1:8088/`) to chat with Spider Mesh and configure the agent. When you're ready to chat in DingTalk, Feishu, QQ, etc., head to [Channels](./channels) to add a channel.
...
configured a channel, Spider Mesh will reply there; otherwise you can add one after
...
If you're not comfortable with command-line tools, you can download and use Spider Mesh's desktop application without manually configuring Python environments or running commands.
...
2. **Install and Launch**
   ...
   - **macOS**: Extract the `.zip` to get `Spider Mesh.app`, first time requires right-click and select "Open" to bypass system security restrictions
...
- **Chat with Spider Mesh** — [Channels](./channels): connect one channel
