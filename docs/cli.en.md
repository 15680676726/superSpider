...
Start the Spider Mesh server. Everything else — channels, cron jobs, the Console
UI — depends on this.
...
Before using Spider Mesh you need at least one LLM provider configured. Environment
...
Spider Mesh can also run models locally via llama.cpp or MLX — no API key needed.
...
Spider Mesh integrates with Ollama to run models locally. Models are dynamically loaded from your Ollama daemon — install Ollama first from [ollama.com](https://ollama.com).
...
- Models come from Ollama daemon (not downloaded by Spider Mesh)
- Use `ollama-pull` / `ollama-remove` instead of `download` / `remove-local`
- Model list updates dynamically when you add/remove via Ollama CLI or Spider Mesh

> **Note:** You are responsible for ensuring the API key is valid. Spider Mesh does
...
> **Note:** Spider Mesh only stores and loads these values; you are responsible for
...
Connect Spider Mesh to messaging platforms.
...
ask Spider Mesh and send the reply". **Requires `copaw app` to be running.**
...
- **agent** — ask Spider Mesh a question on schedule and deliver the reply.
...
# Agent: every 2 hours, ask Spider Mesh and forward the reply
copaw cron create \
  --type agent \
  --name "Check todos" \
  --cron "0 */2 * * *" \
...
Extend Spider Mesh's capabilities with skills (PDF reading, web search, etc.).
...
`HEARTBEAT.md`, legacy files such as `jobs.json` / `chats.json` (no longer
read by the runtime), skills, memory, and agent persona files.
...
- [Introduction](./intro) — What Spider Mesh can do
