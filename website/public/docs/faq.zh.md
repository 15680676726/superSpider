...
> - **重新运行**：打开新终端，再次执行安装脚本以完成Spider Mesh安装。
> - **配置应用 bin 路径**：将 `%USERPROFILE%\.copaw\bin` 添加到系统的 `Path` 变量中。

### Spider Mesh如何更新

要更新 Spider Mesh 到最新版本，可根据你的安装方式选择对应方法：
...
原因1：没有配置模型 API key，需要获取 API key后，在 **控制台 → 系统设置 → 模型** 中配置。
