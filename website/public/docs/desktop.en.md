...
This guide explains how to install and use the Spider Mesh desktop application on Windows and macOS.
...
1. **Download the installer**
   Download `Spider-Mesh-Setup-<version>.exe` from the [Release page][releases]

2. **Run the installer**
   Double-click the `.exe` file and follow the installation wizard
   - Default installation location: `C:\Users\<your-username>\AppData\Local\SpiderMesh`
...
#### **Spider Mesh** (Recommended for daily use)
...
- **How to Launch**: Double-click the "Spider Mesh" icon on desktop or Start Menu
- **Technical Note**: Uses VBScript launcher, runs Python process in background

#### **Spider Mesh (Debug)** (Debug Mode)
...
- **How to Launch**: Double-click "Spider Mesh (Debug)" in Start Menu
- **Log Contents**:
...
A: Use "Spider Mesh (Debug)" to view terminal output for error messages

**Q: How to uninstall?**
A: Go to Windows Settings → Apps → Installed apps → Find "Spider Mesh" → Uninstall
...
1. **Download the archive**
   Download `Spider-Mesh-<version>-macOS.zip` from the [Release page][releases]

2. **Extract**
   Double-click the `.zip` file to extract and get `Spider Mesh.app`

3. **Move to Applications folder (Optional)**
   Drag `Spider Mesh.app` to the `/Applications` folder

### First Launch: Bypassing System Security Restrictions

#### Why manual trust is needed?

The current Spider Mesh desktop build is **not Apple Developer-signed or notarized**, so macOS Gatekeeper will block it by default.
...
1. **Right-click** (or Control + click) on `Spider Mesh.app`
...
2. Scroll down to find the blocked app message for the desktop build
...
xattr -cr /Applications/Spider\ Mesh.app
```
...
- Double-click `Spider Mesh.app` to launch
...
cd /Applications  # or wherever your Spider Mesh.app is located

# Set environment variables and launch
APP_ENV="$(pwd)/Spider Mesh.app/Contents/Resources/env"
...
**Q: How to uninstall?**
A: Drag `Spider Mesh.app` to the Trash, then delete the `~/.copaw` configuration folder
