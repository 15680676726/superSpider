...
启动 Spider Mesh 服务。频道、定时任务、控制台等所有运行时功能都依赖此服务。
...
使用 Spider Mesh 前至少需要配置一个 LLM 提供商。环境变量为内置工具（如网页搜索）提供凭据。
...
Spider Mesh 也支持通过 llama.cpp 或 MLX 在本地运行模型——无需 API Key。
...
Spider Mesh 集成 Ollama 以在本地运行模型。模型从 Ollama 守护进程动态加载——请先从 [ollama.com](https://ollama.com) 安装 Ollama。
...
- 模型来自 Ollama 守护进程（不由 Spider Mesh 下载）
- 使用 `ollama-pull` / `ollama-remove` 而非 `download` / `remove-local`
- 通过 Ollama CLI 或 Spider Mesh 添加/删除模型时，模型列表自动更新

> **注意：** API Key 的有效性需要用户自行保证，Spider Mesh 不会验证。
...
> **注意：** Spider Mesh 只负责存储和加载，值的有效性需要用户自行保证。
...
让 Spider Mesh 按时间自动执行任务——「每天 9 点发消息」「每 2 小时提问并转发回复」。
**需要 `copaw app` 正在运行。**
...
- **text** —— 到点向频道发一段固定文案。
- **agent** —— 到点向 Spider Mesh 提问，把回复发到频道。
...
扩展 Spider Mesh 的能力（PDF 阅读、网页搜索等）。
...
`chats.json` 这类遗留删旧文件、技能文件、记忆文件和 Agent 人设 Markdown 文件。
