...
| `jobs.json`          | Legacy cron file; active runtimes use Phase 1 state (manual cleanup if present) |
| `chats.json`         | Legacy chat file; active runtimes use Phase 1 state (manual cleanup if present) |
...
| `COPAW_JOBS_FILE`                  | `jobs.json`     | Legacy cron file name (deprecated; runtime ignores; manual cleanup only)            |
| `COPAW_CHATS_FILE`                 | `chats.json`    | Legacy chats file name (deprecated; runtime ignores; manual cleanup only)           |
...
Spider Mesh needs an LLM provider to work. You can set it up in three ways:
...
> Spider Mesh does not verify whether the key is correct or has sufficient quota —
...
> keys) are valid. Spider Mesh only stores and injects them — it does not verify
> correctness.
...
| Directory                     | Purpose                                                             |
| ----------------------------- | ------------------------------------------------------------------- |
| Built-in (in source code)     | Shipped with Spider Mesh — docx, pdf, pptx, xlsx, news, email, cron, etc. |
...
Spider Mesh has persistent cross-conversation memory: it automatically compresses context and saves key information to Markdown files for long-term retention. See [Memory](./memory.en.md) for full details.
...
> **Recommended:** Keep `EMBEDDING_FOLLOW_ACTIVE_PROVIDER=true` and `FTS_ENABLED=true`. If your active provider already exposes reusable remote credentials, memory can enable hybrid vector + BM25 retrieval without separate embedding config.
...
>
> The prompt design in this section draws on open-source agent practices.
