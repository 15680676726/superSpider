...
Spider Mesh 所有配置和数据默认都在一个目录里，叫**工作目录**，默认是：
...
| `HEARTBEAT.md`       | 心跳每次要问 Spider Mesh 的内容                     |
| `jobs.json`          | 遗留定时任务删旧文件；运行主链现已改走 Phase 1 state |
| `chats.json`         | 遗留会话删旧文件；运行主链现已改走 Phase 1 state     |
...
| `COPAW_JOBS_FILE`                  | `jobs.json`    | 遗留定时任务删旧文件名（相对工作目录，用于 audit/cleanup）     |
| `COPAW_CHATS_FILE`                 | `chats.json`   | 遗留会话删旧文件名（相对工作目录，用于 audit/cleanup）         |
...
Spider Mesh 需要 LLM 提供商才能运行。有三种设置方式：
...
| ------------------------- | -------- | -------------- | ------------------ |
| 自定义 / OpenAI 兼容提供商 | `custom` | _（你自己填）_ | _（按提供商约定）_ |
...
>
> **注意：** API Key 和 Base URL 的有效性需要用户自行保证。Spider Mesh 不会验证你填入的密钥是否正确或是否有余额，请确保所选提供商和模型可正常访问。
...
> **注意：** 环境变量的值（如第三方 API Key）的有效性需要用户自行保证。Spider Mesh 只负责存储和注入，不会校验其正确性。
...
| 目录                          | 说明                                                           |
| ----------------------------- | -------------------------------------------------------------- |
| 内置（源码中）                | 随 Spider Mesh 一起发布——docx、pdf、pptx、xlsx、news、email、cron 等 |
...
Spider Mesh 拥有跨对话的持久记忆能力：自动压缩上下文，并将关键信息写入 Markdown 文件长期保存。详细说明请看 [记忆](./memory.zh.md)。
...
| 环境变量               | 说明                     | 默认值                                              |
| ---------------------- | ------------------------ | --------------------------------------------------- |
| `EMBEDDING_API_KEY`    | Embedding 服务的 API Key | （空；允许继承时优先复用当前激活提供方）            |
| `EMBEDDING_BASE_URL`   | Embedding 服务的 URL     | 当前激活提供方 Base URL，或系统内置的 OpenAI 兼容默认值 |
| `EMBEDDING_MODEL_NAME` | Embedding 模型名称       | 可识别提供方时自动推断默认值（`text-embedding-v4` / `text-embedding-3-small`） |
| `EMBEDDING_FOLLOW_ACTIVE_PROVIDER` | 当显式 embedding key / base URL 为空时，是否复用当前激活模型提供方 | `true` |
| `EMBEDDING_DIMENSIONS` | 向量维度                 | `1024`                                              |
| `FTS_ENABLED`          | 是否启用 BM25 全文检索   | `true`                                              |

> **推荐**：保持 `EMBEDDING_FOLLOW_ACTIVE_PROVIDER=true` 与 `FTS_ENABLED=true`。如果当前已配置的是受支持的远程模型提供方，通常无需再额外填写 embedding 配置，也能直接启用向量 + BM25 混合检索。
