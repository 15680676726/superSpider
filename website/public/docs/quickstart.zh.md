...
> 💡 **安装并启动后**：在配置频道之前，可先打开 [控制台](./console)（浏览器访问 `http://127.0.0.1:8088/`）与 Spider Mesh 对话、配置 Agent；要在钉钉、飞书、QQ 等 app 里对话时，再前往 [频道配置](./channels) 接入频道。
...
>
>    - **找到安装目录**：
>      - 检查 `uv` 是否可用：在 CMD 中输入 `uv --version` ，如果显示版本号，则**只需配置应用 bin 路径**；如果提示 `'uv' 不是内部或外部命令，也不是可运行的程序或批处理文件。`，则需同时配置两者。
>      - uv路径（任选其一，取决于安装位置，若`uv`不可用则填）：通常在`%USERPROFILE%\.local\bin`、`%USERPROFILE%\AppData\Local\uv`或 Python 安装目录下的 `Scripts` 文件夹
>      - 应用 bin 路径：通常在 `%USERPROFILE%\.copaw\bin` 。
>    - **手动添加到系统的 Path 环境变量**：
...
> - **重新运行**：打开新终端，再次执行安装脚本以完成 Spider Mesh 安装。
> - **配置应用 bin 路径**：将 `%USERPROFILE%\.copaw\bin` 添加到系统的 `Path` 变量中。
...
服务默认监听 `127.0.0.1:8088`。若已配置频道，Spider Mesh 会在对应 app 内回复；若尚未配置，也可先完成本节再前往频道配置。
...
如果你不习惯使用命令行，可以下载并使用 Spider Mesh 的桌面应用版本，无需手动配置 Python 环境或执行命令。
...
2. **安装并启动**
   ...
   - **macOS**: 解压 `.zip` 得到 `Spider Mesh.app`，首次需右键选择"打开"以绕过系统安全限制
...
- **想和 Spider Mesh 对话** → 去 [频道配置](./channels) 接一个频道（推荐先接钉钉或飞书），按文档申请应用、填 config，保存后即可在对应 app 里发消息试。
