...
def test_industry_operating_cycle_emits_cycle_prediction_opportunities(tmp_path) -> None:
    ...
    execution_backlog = app.state.industry_service._backlog_service.record_chat_writeback(
        industry_instance_id=instance_id,
    ...
    case_detail = app.state.prediction_service.get_case_detail(
        processed["created_prediction_case_id"],
    )
    planning = case_detail.case["planning"]
    assert planning["source"] == "formal-cycle-review-overlap"
    assert execution_backlog.id in planning["selected_backlog_item_ids"]
    assert execution_lane["lane_id"] in planning["selected_lane_ids"]
    assert planning["review_ref"].startswith(
        f"formal-cycle-review|{instance_id}|",
    )
    assert planning["replan"]["status"] == "clear"
