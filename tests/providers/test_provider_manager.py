...
def test_set_fallback_config_persists_and_reloads(isolated_secret_dir) -> None:
    manager = ProviderManager()
    local_provider = manager.get_provider("llamacpp")
    assert local_provider is not None
    local_provider.models = [ModelInfo(id="llama-local", name="Llama Local")]

    config = manager.set_fallback_config(
        enabled=True,
        candidates=[
            provider_manager_module.ModelSlotConfig(
    ...
    reloaded = ProviderManager()
    slots = reloaded.get_fallback_slots()
    assert len(slots) == 1
    assert slots[0].provider_id == "llamacpp"
    assert slots[0].model == "llama-local"


def test_set_fallback_config_rejects_unknown_candidate(isolated_secret_dir) -> None:
    manager = ProviderManager()

    with pytest.raises(ValueError, match="Fallback provider 'missing' not found"):
...
def test_get_active_models_info_reports_fallback_resolution(isolated_secret_dir) -> None:
    manager = ProviderManager()
    local_provider = manager.get_provider("llamacpp")
    assert local_provider is not None
    local_provider.models = [ModelInfo(id="llama-local", name="Llama Local")]
    manager.active_model = provider_manager_module.ModelSlotConfig(
        provider_id="openai",
        model="gpt-5",
    )
    manager.set_fallback_config(
    ...
    info = manager.get_active_models_info()

    assert info.active_llm is not None
    assert info.resolved_llm is not None
    assert info.resolved_llm.provider_id == "llamacpp"
    assert info.fallback_applied is True
    assert info.fallback_chain[0].provider_id == "llamacpp"
    assert any("openai/gpt-5: api key missing" in item for item in info.unavailable_candidates)


@pytest.mark.asyncio
async def test_get_active_chat_model_uses_runtime_fallback_when_primary_creation_fails(
    isolated_secret_dir,
    monkeypatch,
) -> None:
    manager = ProviderManager()
    manager.update_provider("openai", {"api_key": "sk-primary"})
    ...
    ProviderManager._instance = manager
    ...
    try:
        resolved = ProviderManager.get_active_chat_model()
    finally:
        ProviderManager._instance = None

    stream = await resolved(messages=[{"role": "user", "content": "hello"}])
    payloads = [item async for item in stream]
    assert payloads == [{"provider_id": "dashscope", "model": "qwen3-max"}]


@pytest.mark.asyncio
async def test_runtime_fallback_handles_first_token_failure(
    isolated_secret_dir,
    monkeypatch,
) -> None:
    manager = ProviderManager()
    ...
    class _FallbackStreamModel:
        async def __call__(self, *args, **kwargs):
            async def _stream():
                yield {"provider_id": "dashscope", "model": "qwen3-max", "ok": True}
    ...
    payloads = [item async for item in stream]
    assert payloads == [
        {"provider_id": "dashscope", "model": "qwen3-max", "ok": True},
    ]
