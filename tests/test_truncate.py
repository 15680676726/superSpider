...
def test_shell_normal_output(shell_test_dir):
    ...
    result = asyncio.run(
        shell.execute_shell_command(_python_command("print('hello')"), cwd=shell_test_dir),
    )
...
def test_shell_multiline_output(shell_test_dir):
    ...
    result = asyncio.run(
        shell.execute_shell_command(
            _python_command("for i in range(1, 11): print(i)"),
            cwd=shell_test_dir,
        ),
    )
    text = result.content[0].get("text", "")
...
def test_shell_truncated_by_lines(shell_test_dir):
    ...
    cmd = _python_command(
        f"for i in range(1, {lines_to_generate + 1}): print(i)",
    )
...
def test_shell_truncated_by_bytes(shell_test_dir):
    ...
    cmd = _python_command(
        f"for _ in range({lines_needed}): print('{ 'x' * 100 }')",
    )
    result = asyncio.run(shell.execute_shell_command(cmd, cwd=shell_test_dir))
    text = result.content[0].get("text", "")
