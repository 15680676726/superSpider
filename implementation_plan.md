...
- `2026-04-17` universal external information collection 补充：研究链已不再只等于 `BaiduPageResearchService` 单线。当前正式主链新增 `source_collection` 编排层，稳定动作模型收口为 `discover / read / interact / capture`，`search / web_page / github / artifact` 已作为 phase-1 adapters 落地，并支持 live single-source collection；真实外部多轮采集默认仍由 heavy `BaiduPageResearchService` 承接，但它已不再等于整套 universal collection model
...
- `2026-04-17` research truth 补充：`ResearchSessionRecord` 现已正式持久化 session-level `brief / conflicts / writeback_truth`，`ResearchSessionRoundRecord` 现已正式持久化 round-level `sources / findings / conflicts / gaps / writeback_truth`；Runtime Center 读面优先消费这些 top-level persisted projections，不再主要依赖 `stable_findings / open_questions / metadata` 聚合投影
- `2026-04-17` writeback 边界补充：heavy `BaiduPageResearchService.summarize_session()` 继续把研究结果写成 `AgentReportRecord` 并把摘要写入 `work_context / industry` 知识面；与此同时，light inline collection 现已在 live frontdoor 上自动写 dedicated `EvidenceRecord`，并自动应用 report / knowledge / graph writeback。`StateKnowledgeService.ingest_research_session(...)` 与 `KnowledgeWritebackService.build_research_session_writeback(...)` 当前已经成为 heavy/light 共用 builder
