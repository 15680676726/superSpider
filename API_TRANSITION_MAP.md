...
- `V4` 的角色治理语义还应继续收紧：`execution-core` 产品上应呈现为团队总控核，推荐能力集以 dispatch / delegate / supervise / summarize 为主；外部执行能力推荐应优先出现在专业 agent 的 capability surface 中
  ...
  - `2026-04-17` 边界补充：当前统一前门已保证 `ResearchSessionRecord.brief + ResearchSessionRoundRecord.sources + Runtime Center read surface` 成为 formal truth；`findings / gaps / conflicts` 当前仍主要通过 `stable_findings / open_questions / metadata` 聚合投影，light inline collection 也还没有统一新写 dedicated `EvidenceRecord`。此外，inline light collection 还没有自动把 knowledge / graph writeback 应用到 live frontdoor，当前 report/knowledge summary 的正式 writeback owner 仍是 heavy `summarize_session()` 与可复用的 research writeback builders
  - `2026-04-17` adapter 边界补充：`search / web_page / github / artifact` 已作为 phase-1 typed adapters 落地，但当前 light adapters 仍主要是 metadata-backed shapers；真实外部多轮采集 owner 仍是 heavy `BaiduPageResearchService`
