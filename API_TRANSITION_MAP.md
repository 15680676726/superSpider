...
- `2026-03-25` 完成补充：
  ...
  - `2026-04-03` 六缺口收口补充：provider 的正式写前门现已收口到 `/providers/admin/*`，`/models` 只保留读面；`local_models` 刷新 catalog 也统一走 `ProviderAdminService`，不再直写 provider runtime 状态。runtime/bootstrap 正式 contract 现只暴露 `runtime_provider`，`provider_manager` 只允许留在 `runtime_state_bindings.py` 的 compatibility mirror；`Runtime Center` 页面对后端也已统一改走 canonical `/runtime-center/surface`，前端不再并行请求 `overview + main-brain + businessAgents`。与此同时，`industry/service_runtime_views.py` 的 live focus 读面不再从 assignment/backlog/task 读时发明 `current_focus_*`，只透传 runtime 正式 focus；`/runtime-center/main-brain` 的 dedicated payload 也不再用 overview entry 伪造 `strategy/carrier` truth，cockpit 自身也不再回退读取 overview `surface/generated_at/error`。
  - `2026-04-04` 续补：`Runtime Center` 的 automation/recovery surface 现已进一步从“typed facade 包装”收紧到正式 snapshot seam。`ActorSupervisor.snapshot()`、`AutomationTaskGroup.overview_snapshot()` 与 `RuntimeCenterAppStateView.resolve_recovery_summary()` 已成为 canonical read contract；`overview_cards.py` 不再直接读取 `_loop_task / _agent_tasks` 私有字段，`/runtime-center/recovery/latest` 与 `/system/*` 也统一经 shared recovery resolver 读取 recovery truth。前端 `Runtime Center` 对 SSE topic 的 section 刷新也已拒绝 unknown-topic -> full-surface reload 回退，`Sidebar` 的 `agents` 顶层 IA 已物理删除，不再把 `/agents` 当成第二个正式 runtime 首页。
...
- 迁移说明：
  ...
  - `2026-04-01` 补充：下层 execute contract 已完成一轮 hardening。`CapabilityExecutionContext` 只作为内部执行上下文存在，不升格成第二真相源；`CapabilityExecutionFacade.execute_task()` 现统一返回规范化 result shape，显式暴露 `error_kind / action_mode / work_context_id / evidence_id`，且早返回 / 异常 / tool-response 三条路径不再漂移
...
- `2026-04-02` 补充：`GET /api/runtime-center/conversations/{thread_id}` 现在允许把同线程 session snapshot 中的 `main_brain.phase2_commit` 投影为 `meta.main_brain_commit`，用于聊天页刷新后的同线程 commit state reload；该字段只是 canonical session snapshot 的派生读面，不新增第二真相源。
- `2026-04-02` 补充：runtime chat media 的 adopt / retain / writeback 现在会沿正式 `work_context / industry / agent` scope 解析链标记主脑 scope snapshot dirty；snapshot refresh 已收口为增量刷新，不再在每轮媒体写回后全量重建 truth-first memory prompt。
...
- `V4` 的角色治理语义还应继续收紧：`execution-core` 产品上应呈现为团队总控核，推荐能力集以 dispatch / delegate / supervise / summarize 为主；外部执行能力推荐应优先出现在专业 agent 的 capability surface 中
  ...
  - `2026-04-17` 边界补充：当前统一前门已保证 `ResearchSessionRecord.brief / conflicts / writeback_truth + ResearchSessionRoundRecord.sources / findings / conflicts / gaps / writeback_truth + Runtime Center read surface` 成为 formal truth；light inline collection 现在也会统一新写 dedicated `EvidenceRecord`，并自动把 report / knowledge / graph writeback 应用到 live frontdoor。当前 report/knowledge summary 的正式 writeback owner 不再只剩 heavy `summarize_session()`，而是由 heavy summary 与共用 research writeback builders 共同承担
  - `2026-04-17` adapter 边界补充：`search / web_page / github / artifact` 已作为 phase-1 typed adapters 落地，并支持 live single-source collection；真实外部多轮采集默认仍由 heavy `BaiduPageResearchService` 承接，但它已经通过 `collect_via_baidu_page(...)` 被收进共享 provider adapter seam，不再等于整套 universal collection model
  ...
  - `2026-04-20` searching substrate 补充：research frontdoor 现已在 `SourceCollectionService` 下游新增 `RetrievalFacade`，phase-1 source seam 固定为 `local_repo / github / search / web_page`；selected retrieval hits 统一先归一为 `RetrievalHit`，再映射回 `CollectedSource / ResearchFinding / ResearchAdapterResult`
  - `2026-04-20` 边界补充：`RetrievalQuery / RetrievalPlan / RetrievalHit / RetrievalRun / RepositoryIndexSnapshot / CodeSymbolRecord` 只属于 retrieval substrate，可重建、可清空、不可成为第二 formal truth；canonical truth 仍然是 `ResearchSession* / Evidence / report / knowledge / graph writeback`
  - `2026-04-20` read surface 补充：`GET /runtime-center/research` 现已额外暴露 retrieval explanation：`intent / requested_sources / mode_sequence / coverage / selected_hits / dropped_hits / trace`；但现役读链仍保持 top-level formal fields 优先、metadata fallback 次之，不允许为了 retrieval trace 把 formal `sources/findings/conflicts/gaps/writeback_truth` 再降回隐式 metadata
  - `2026-04-20` heavy-boundary 补充：`artifact-followup` 与 heavy `BaiduPageResearchService` 仍保留原有 owner 身份；retrieval substrate 只增强 `light`/typed adapter 检索与 Runtime Center explanation，不回退 heavy external collection 主链
...
- `2026-04-03` 补充：`/runtime-center/main-brain` 现已把 `main_brain_planning` 提升成同名 dedicated read contract；主脑 cockpit 直接结构化消费 `strategy_constraints / latest_cycle_decision / focused_assignment_plan / replan`，不再只借道 `report_cognition.replan` 暴露 planning shell
