...
- `2026-03-25` 完成补充：
  ...
  - `2026-04-03` 补充：industry runtime writer 也已切掉最后一条 runtime metadata 双写；`industry/service_team_runtime.py` 不再把 `goal_id / goal_title` 持久化进 `AgentRuntimeRecord.metadata`，因此 bootstrap、lifecycle refresh 与 cleanup backfill 现在都会统一只落 `current_focus_*`。legacy `goal_*` 仅剩 `AgentProfileService` 的历史兼容读，不再属于 live runtime writer contract
