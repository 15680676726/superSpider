...
install_qmd_sidecar() {
    if command -v qmd &>/dev/null; then
        info "QMD sidecar already installed"
        return
    fi
    if ! command -v npm &>/dev/null; then
        warn "npm not found - skipping built-in QMD install."
        warn "Install Node.js from https://nodejs.org/ then run: npm install -g @tobilu/qmd"
        return
    fi
    info "Installing built-in QMD sidecar (npm install -g @tobilu/qmd)..."
    if npm install -g @tobilu/qmd; then
        if command -v qmd &>/dev/null; then
            info "QMD sidecar installed successfully"
            return
        fi
        warn "QMD install finished but qmd is not on PATH yet."
        return
    fi
    warn "QMD install failed. CoPaw will keep running with local memory backends."
}
