...
class _IndustryLifecycleMixin:
    def __init__(
        ...
        *,
        ...
        self._operating_cycle_repository = operating_cycle_repository
        ...
        if self._state_store is not None:
            if self._operating_lane_repository is None:
                self._operating_lane_repository = SqliteOperatingLaneRepository(
                    self._state_store,
                )
            if self._backlog_item_repository is None:
                self._backlog_item_repository = SqliteBacklogItemRepository(
                    self._state_store,
                )
            if self._operating_cycle_repository is None:
                self._operating_cycle_repository = SqliteOperatingCycleRepository(
                    self._state_store,
                )
            if self._assignment_repository is None:
                self._assignment_repository = SqliteAssignmentRepository(
                    self._state_store,
                )
            if self._agent_report_repository is None:
                self._agent_report_repository = SqliteAgentReportRepository(
                    self._state_store,
                )
        self._operating_lane_service = operating_lane_service or (
            OperatingLaneService(repository=self._operating_lane_repository)
            if self._operating_lane_repository is not None
            else None
        )
        self._backlog_service = backlog_service or (
            BacklogService(repository=self._backlog_item_repository)
            if self._backlog_item_repository is not None
            else None
        )
        self._operating_cycle_service = operating_cycle_service or (
            OperatingCycleService(repository=self._operating_cycle_repository)
            if self._operating_cycle_repository is not None
            else None
        )
        self._assignment_service = assignment_service or (
            AssignmentService(repository=self._assignment_repository)
            if self._assignment_repository is not None
            else None
        )
        self._agent_report_service = agent_report_service or (
            AgentReportService(
    ...
    def _public_bootstrap_activation_flags(
        ...
        - Public bootstrap only creates the formal industry identity and parks it at
          the execution-core front door.
        - Learning and execution must start later from explicit main-brain chat kickoff,
          not from bootstrap flags.
        """
    ...
    async def add_role_to_instance_team(
        ...
        *,
        ...
        elif role.employment_mode != "temporary":
    ...
    def list_instances(
        ...
        *,
        ...
        summaries = [
            ...
            if (summary.stats.get("goal_count") or 0) > 0
        ]
        summaries.sort(
    ...
    def count_instances(self) -> int:
        return sum(
            ...
            if (self.reconcile_instance_status(record.instance_id) or record).status == "active"
            and len(record.goal_ids or []) > 0
        )
    ...
    def reconcile_instance_status_for_goal(self, goal_id: str) -> None:
        ...
        if not normalized_goal_id:
            return
        for record in self._industry_instance_repository.list_instances(status=None):
            if normalized_goal_id in set(record.goal_ids or []):
    ...
    async def kickoff_execution_from_chat(
        ...
        *,
        ...
        _ = message_text, session_id, channel
        ...
        if kickoff_stage == "learning" and callable(acquisition_runner):
            try:
                acquisition_cycle = await acquisition_runner(
                    industry_instance_id=record.instance_id,
                ...
                acquisition_cycle = {
                    "success": False,
                    "industry_instance_id": record.instance_id,
                    "summary": str(exc).strip() or "自动装配学习执行失败。",
                    "proposals": [],
                    "plans": [],
                    "onboarding_runs": [],
    ...
    async def apply_execution_chat_writeback(
        ...
        *,
        ...
        target_role, target_match_signals = self._resolve_chat_writeback_target_role(
            record=record,
            team=team,
        ...
        target_owner_agent_id = (
            ...
            else resolved_execution_owner_agent_id
        )
        target_industry_role_id = (
            target_role.role_id
            if target_role is not None
            else EXECUTION_CORE_ROLE_ID
        )
        target_goal_kind = (
        ...
        target_role_name = (
            _string(target_role.role_name)
            if target_role is not None
            else execution_core_identity.role_name
        ) or execution_core_identity.role_name
        target_role_summary = (
            _string(target_role.role_summary)
            if target_role is not None
            else execution_core_identity.role_summary
        ) or execution_core_identity.role_summary
        target_mission = (
            _string(target_role.mission)
            if target_role is not None
            else execution_core_identity.mission
        ) or execution_core_identity.mission
        target_environment_constraints = (
            list(target_role.environment_constraints)
            if target_role is not None
            else list(execution_core_identity.environment_constraints)
        )
        target_evidence_expectations = (
            list(target_role.evidence_expectations)
            if target_role is not None
            else list(execution_core_identity.evidence_expectations)
        )
        target_task_mode = infer_industry_task_mode(
            role_id=target_industry_role_id,
        ...
        initial_strategy_classifications = list(plan.classifications)
        ...
        if callable(retain):
            try:
                retain(
                    ...
                    source_ref=f"chat-writeback:{plan.fingerprint}",
                    role_bindings=[target_industry_role_id] if target_industry_role_id else [],
                    tags=list(plan.classifications),
                )
            except Exception:
                pass
        dispatch_deferred = bool(created_backlog_ids and target_role is None)
        if dispatch_deferred:
            plan.classifications = _unique_strings(
                list(plan.classifications),
                ["routing-pending"],
            )
        if created_backlog_ids and target_role is not None:
            cycle_result = await self.run_operating_cycle(
                instance_id=active_record.instance_id,
                actor="industry-chat-writeback",
                force=True,
                limit=1,
            ...
            if refreshed_record is not None:
                active_record = refreshed_record
            created_goal_ids = []
            created_goal_titles = []
            if processed_instances:
                created_goal_ids = _unique_strings(
                    processed_instances[0].get("created_goal_ids"),
                )
                for goal_id in created_goal_ids:
                    goal = self._goal_service.get_goal(goal_id)
                    if goal is not None:
                        created_goal_titles.append(goal.title)
                if created_goal_ids:
                    plan.classifications = _unique_strings(
                        list(plan.classifications),
                        ["immediate-goal"],
                    ...
                    if (
                        self._strategy_memory_service is not None
                        and merged_strategy is not None
                        and plan.classifications != initial_strategy_classifications
                    ):
                        refreshed_strategy = self._merge_chat_writeback_strategy(
                            record=active_record,
                            profile=updated_profile,
                            team=team,
                            execution_core_identity=execution_core_identity,
                            existing_strategy=merged_strategy,
                            plan=plan,
                        )
                        if self._strategy_memory_changed(merged_strategy, refreshed_strategy):
                            self._strategy_memory_service.upsert_strategy(refreshed_strategy)
                            strategy_updated = True
                        merged_strategy = refreshed_strategy
                        strategy_id = refreshed_strategy.strategy_id
                    ...
                    for goal_id in created_goal_ids:
                        dispatch = await self._goal_service.dispatch_goal(
                            ...
                            context={
                                ...
                                "chat_writeback_classes": list(plan.classifications),
                                "chat_writeback_target_role_name": target_role_name,
                                "chat_writeback_target_match_signals": list(
                        ...
                        goal_dispatches.append(dispatch)
        if target_role is not None and created_goal_ids:
            target_status = self._resolve_chat_writeback_agent_status(role=target_role)
            for goal_id, goal_title in zip(created_goal_ids, created_goal_titles):
                self._upsert_agent_profile(
                    target_role,
                    instance_id=active_record.instance_id,
                    goal_id=goal_id,
        ...
        return {
            ...
            "delegated": bool(
                ...
                and not is_execution_core_role_id(target_industry_role_id)
            ),
        }
    ...
    def _run_operating_cycle_for_instance(
        ...
        *,
        ...
        if task_repository is not None:
            for goal_id in record.goal_ids:
                goal_tasks = task_repository.list_tasks(goal_id=goal_id)
                if goal_tasks:
                    tasks_by_goal_id[goal_id] = list(goal_tasks)
        ...
        if should_start and open_backlog:
            ...
            updated_goal_ids = list(record.goal_ids or [])
            for item in selected_backlog:
                materialized_goal = (
                    self._goal_service.get_goal(item.goal_id)
                    if item.goal_id
                    else None
                )
                if materialized_goal is None or materialized_goal.status in {"completed", "archived"}:
                    materialized_goal = self._materialize_backlog_goal(
                        record=record,
                        item=item,
                        cycle=new_cycle,
                    )
                    new_goal_ids.append(materialized_goal.id)
                elif materialized_goal.cycle_id != new_cycle.id:
                    materialized_goal = self._goal_service.update_goal(
                        materialized_goal.id,
                        status=(
                            "paused"
                        ...
                        goal_class=materialized_goal.goal_class,
                    )
                if materialized_goal.id not in updated_goal_ids:
                    updated_goal_ids.append(materialized_goal.id)
                assignment_specs.append(
                    {
                        "goal_id": materialized_goal.id,
                        "backlog_item_id": item.id,
                        "lane_id": item.lane_id,
                        "owner_agent_id": _string(item.metadata.get("owner_agent_id")),
                        "owner_role_id": _string(item.metadata.get("industry_role_id")),
                        "title": materialized_goal.title,
                        "summary": materialized_goal.summary,
                        "goal_status": materialized_goal.status,
                        "report_back_mode": _string(item.metadata.get("report_back_mode")) or "summary",
                        "metadata": {
                            "source_ref": item.source_ref,
                            "source_kind": item.source_kind,
                            "sop_binding_id": _string(item.metadata.get("sop_binding_id")),
                            "sop_binding_name": _string(item.metadata.get("sop_binding_name")),
            ...
            new_cycle = self._operating_cycle_service.update_cycle_links(
                new_cycle,
                goal_ids=_unique_strings(list(new_cycle.goal_ids or []), updated_goal_ids),
                ...
                self._backlog_service.mark_item_materialized(
                    ...
                    goal_id=next(
                        (
                            ...
                            if spec.get("backlog_item_id") == item.id
                        ),
                        None,
                    ),
            ...
            record = self._industry_instance_repository.upsert_instance(
                record.model_copy(
                    update={
                        "goal_ids": _unique_strings(list(record.goal_ids or []), new_goal_ids),
        ...
        *,
        ...
        for goal_id in record.goal_ids:
            for task in task_repository.list_tasks(goal_id=goal_id):
                ...
                assignment = assignments.get(task.assignment_id or "") or assignments_by_goal.get(goal_id)
                evidence_ids: list[str] = []
                task_routine_run_id: str | None = None
                task_sop_binding_id: str | None = None
                task_sop_workflow_run_id: str | None = None
                if self._evidence_ledger is not None:
                    for evidence in self._evidence_ledger.list_by_task(task.id):
                        ...
                        nested_sop_binding_id = (
                            _string(evidence_meta.get("sop_binding_id"))
                            or _string(evidence_meta.get("binding_id"))
                        )
                        if task_sop_binding_id is None and nested_sop_binding_id is not None:
                            task_sop_binding_id = nested_sop_binding_id
                        nested_sop_workflow_run_id = (
                            _string(evidence_meta.get("sop_workflow_run_id"))
                            or _string(evidence_meta.get("workflow_run_id"))
                        )
                        if (
                            task_sop_workflow_run_id is None
                            and nested_sop_workflow_run_id is not None
                        ):
                            task_sop_workflow_run_id = nested_sop_workflow_run_id
                        nested_sop_evidence_ids = evidence_meta.get("sop_evidence_ids")
                        if isinstance(nested_sop_evidence_ids, list):
                            for nested_id in nested_sop_evidence_ids:
                                nested_text = _string(nested_id)
                                if nested_text is not None and nested_text not in evidence_ids:
                                    evidence_ids.append(nested_text)
                decision_ids = (
                ...
                report = self._agent_report_service.record_task_terminal_report(
                    ...
                    metadata={
                        ...
                        for key, value in {
                            ...
                            "sop_binding_id": task_sop_binding_id,
                            "workflow_run_id": task_sop_workflow_run_id,
                        }.items()
        ...
        messages.append(
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": self._build_agent_report_control_thread_message(
                            report=report,
                            assignment=assignment,
                        ),
                    },
    ...
    def _materialize_backlog_goal(
        ...
        *,
        ...
        trigger_reason = _string(metadata.get("trigger_reason")) or (
            ...
            if source == "chat-writeback"
            else "Operating cycle materialized backlog into an active goal."
        )
        role_id = _string(metadata.get("industry_role_id"))
        owner_agent_id = _string(metadata.get("owner_agent_id"))
        lane = (
        ...
        self._goal_override_repository.upsert_override(
            GoalOverrideRecord(
                ...
                compiler_context={
                    ...
                    "owner_agent_id": owner_agent_id or (lane.owner_agent_id if lane is not None else EXECUTION_CORE_AGENT_ID),
                    ...
                    "lane_id": item.lane_id,
                    "cycle_id": cycle.id,
                    "sop_binding_id": _string(metadata.get("sop_binding_id")),
                    "sop_binding_source_ref": _string(metadata.get("sop_binding_source_ref"))
                    or item.source_ref
                    or item.id,
                    "sop_binding_source_type": _string(metadata.get("sop_binding_source_type"))
                    or "assignment",
                    "sop_binding_risk_level": _string(metadata.get("sop_binding_risk_level")),
                    "sop_binding_input_payload": (
                        dict(metadata.get("sop_binding_input_payload"))
                        if isinstance(metadata.get("sop_binding_input_payload"), dict)
                        else {}
                    ),
                    "sop_binding_metadata": (
                        dict(metadata.get("sop_binding_metadata"))
                        if isinstance(metadata.get("sop_binding_metadata"), dict)
                        else {
                            key: value
                            for key, value in {
                                "sop_binding_name": _string(metadata.get("sop_binding_name")),
                                "backlog_item_id": item.id,
                            }.items()
                            if value is not None
                        }
                    ),
                    "sop_workflow_run_id": _string(metadata.get("sop_workflow_run_id")),
    ...
    async def _prepare_preview(self, request: IndustryPreviewRequest) -> _IndustryPlan:
        ...
        recommendation_pack = await self._build_recommendation_pack(
            profile=profile,
            deferred_message="Preview only creates the initial roles, goals, and schedule drafts. Capability, MCP, and workflow setup will continue after creation.",
            include_install_templates=False,
            include_remote_sources=False,
            deferred_message="预览阶段只生成角色、目标与节奏草案；技能、MCP 与工作流将在身份创建后由主脑结合学习上下文继续配置。",
        )
    ...
    async def _prepare_bootstrap(self, request: IndustryBootstrapRequest) -> _IndustryPlan:
        ...
        recommendation_pack = await self._build_recommendation_pack(
            profile=profile,
            deferred_message="Carrier creation skips immediate capability, MCP, and workflow matching. The main brain will continue that setup after creation.",
            include_install_templates=False,
            include_remote_sources=False,
            deferred_message="身份创建阶段已跳过同步技能、MCP 与工作流匹配；创建后由主脑结合学习上下文继续配置。",
        )
        readiness_checks = self._build_readiness_checks(
            team=draft.team,
            schedule_count=len(schedule_seeds),
    ...
    async def _prepare_team_update(
        ...
        *,
        ...
        recommendation_pack = await self._build_recommendation_pack(
            profile=profile,
            deferred_message="Team update skips immediate capability, MCP, and workflow matching. The main brain will continue that setup after the update.",
            include_install_templates=False,
            include_remote_sources=False,
            deferred_message="团队更新阶段已跳过同步技能、MCP 与工作流匹配；更新后由主脑结合学习上下文继续配置。",
        )
        readiness_checks = self._build_readiness_checks(
            team=draft.team,
            schedule_count=len(schedule_seeds),
