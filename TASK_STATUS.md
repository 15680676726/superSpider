...
- `2026-04-03` 补充：多执行位能力自治的正式设计与施工计划已补齐到仓库文档基线：`docs/superpowers/specs/2026-04-03-multi-seat-capability-autonomy-design.md` 与 `docs/superpowers/plans/2026-04-03-multi-seat-capability-autonomy-implementation-plan.md`。当前正式口径已明确区分 `全局能力底座 / 角色原型能力包 / 执行位实例能力包 / 周期增量包 / session overlay` 五层，不再允许把 skill/MCP 治理继续理解为“每个任务都重跑推荐包”或“一个角色只对应一套静态能力面”。主脑在这条链上的职责也已正式收敛为能力治理决策：判断当前缺口应复用、挂载、安装、升格、替换还是回滚，而不是自己背满执行技能；自动链与人工 `merge / replace` front-door 也被定义为同一条正式写链，而不是两套平行配置路径。
