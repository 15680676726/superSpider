...
- `2026-04-02` 补充：formal planning capability gap 的 `FP-6 / FP-7 / FP-8` 也已在同日收口完成，不再保留 “uncertainty register / strategy-change trigger / multi-cycle lane budget” 尾巴。当前正式读法已收敛为 `strategy compiler -> cycle planner -> assignment planner -> report-to-replan engine`，并且 typed `StrategicUncertaintyRecord / LaneBudgetRecord / ReportReplanDecision.decision_kind` 已贯通 `StrategyMemory -> Goal compiler context -> Industry cycle/assignment formal_planning sidecar -> Prediction planning snapshot -> Runtime Center / main-brain cognition / prompt runtime context`。本轮新增 focused 验收：
  - `$env:PYTHONPATH='src'; python -m pytest tests/compiler/test_planning_models.py tests/compiler/test_strategy_compiler.py tests/compiler/test_cycle_planner.py tests/compiler/test_report_replan_engine.py tests/industry/test_report_synthesis.py tests/state/test_strategy_memory_service.py -q` -> `32 passed`
  - `$env:PYTHONPATH='src'; python -m pytest tests/app/test_goals_api.py::test_compile_goal_injects_strategy_memory_context tests/app/test_predictions_api.py::test_prediction_cycle_case_exposes_light_formal_planning_context_in_detail tests/app/industry_api_parts/runtime_updates.py::test_runtime_detail_exposes_stable_main_brain_planning_surface_from_formal_sidecars tests/app/runtime_center_api_parts/overview_governance.py::test_runtime_center_main_brain_route_exposes_report_cognition_surface tests/kernel/test_main_brain_chat_service.py::test_main_brain_chat_service_prompt_includes_structured_cognitive_closure_state tests/kernel/test_turn_executor.py::test_main_brain_orchestrator_runtime_context_carries_cognitive_surface tests/app/test_phase_next_autonomy_smoke.py::test_phase_next_industry_long_run_smoke_keeps_handoff_human_assist_and_replan_on_one_control_thread -q` -> `7 passed`
  - `$env:PYTHONPATH='src'; python -m pytest tests/app/test_predictions_api.py -q` -> `13 passed`
  - `$env:PYTHONPATH='src'; python -m pytest tests/app/runtime_center_api_parts/overview_governance.py tests/kernel/test_main_brain_chat_service.py tests/kernel/test_turn_executor.py -q` -> `131 passed`
...
- `2026-04-02` 补充：`cc/cowork` donor 对齐又推进了一轮真实主链落地，而不只是文档映射。`QueryExecutionRuntime` 现已在进入长执行前记录正式 `accepted` persistence boundary，并把 `accepted_persistence / commit_outcome` 写回 runtime context；runtime chat sidecar 现已显式区分 `accepted / reply_done / commit_started / confirm_required / committed / commit_failed`，不会再在已知 writeback 失败时继续乐观发 `committed`。同时，`Runtime Center` 已新增正式 bridge lifecycle ops：`ack / heartbeat / stop / reconnect / archive / deregister` 可直接驱动同一条 `EnvironmentMount / SessionMount` 真相链，`health_service` 也已把 `bridge_work_id / bridge_work_status / bridge_heartbeat_at / bridge_session_id / bridge_stopped_at / bridge_stop_mode / workspace_trusted / elevated_auth_state` 投影进 `seat_runtime / host_companion_session.bridge_registration`。另外，desktop app projection 现已对 prompt-facing `app_identity / window_anchor_summary` 做 sanitization；`surface_control_service.execute_windows_app_action()` 也已正式补上 execution-time `operator abort / host exclusion / frontmost verification / clipboard roundtrip verification` guardrail，并通过 runtime event bus 发布 `desktop.guardrail-blocked`。当前剩余 gap 主要变成：donor-grade chat flush durability、external bridge producer 端到端接线，以及 browser/document execution 的 guardrail parity。
