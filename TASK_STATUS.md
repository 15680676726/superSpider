...
- `2026-04-18` 验收补充：按 `UNIFIED_ACCEPTANCE_STANDARD.md` 继续补跑了 `L3` 真实链路 smoke，而不再只停在 focused regression。当前通过的 `L3` 证据包括：`python -m pytest tests/app/test_runtime_canonical_flow_e2e.py tests/app/test_operator_runtime_e2e.py -q` -> `16 passed`，覆盖 `/runtime-center/chat/run`、canonical runtime e2e、operator/runtime e2e 与 Runtime Center 读面主链；`python -m pytest tests/app/test_phase_next_autonomy_smoke.py::test_phase_next_runtime_center_overview_surfaces_main_brain_cockpit_card tests/app/test_phase_next_autonomy_smoke.py::test_phase_next_runtime_cockpit_preserves_evidence_decisions_patches_and_replays_across_reentry -q` -> `2 passed`，补到了 Runtime Center cockpit 重进/重放读链。与此同时，扩面 smoke `python -m pytest tests/app/test_phase_next_autonomy_smoke.py -q -k "runtime_center or runtime_center_overview or canonical_flow or operator or chat_run or long_run_smoke"` 当前为 `2 failed, 2 passed`；两处失败都不是本轮 `日报 + 统一模型调用` 数据合同断层，而是旧 smoke 断言仍在匹配英文文案，实际运行链当前已经返回中文：1) `test_phase_next_runtime_center_overview_surfaces_routine_degradation_contract` 仍断言 `"canonical state only"`，但实际 `routines.meta.remediation_summary` 已是中文文案；2) `test_phase_next_industry_long_run_smoke_keeps_handoff_human_assist_and_replan_on_one_control_thread_contract` 仍断言 `"Runtime handoff is active"`，但实际 governance admission reason 已是中文文案。当前因此只能诚实声明：本轮 `日报/统一模型调用` 已过 `L1 + L2 + 部分 L3`，尚未完成与 phase-next 扩面 smoke 完全一致的 `L3` 收口，更没有 `L4` long soak。
