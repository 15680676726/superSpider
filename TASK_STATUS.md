...
- `2026-04-04` 施工补充：donor-first 外部能力吸纳的第一批正式对象和主读面已进主线，不再只有 `candidate / trial / lifecycle` 的薄壳。`CapabilityCandidateRecord` 现已正式携带 `donor_id / package_id / source_profile_id`；state 层新增了 `CapabilityDonorRecord / CapabilityPackageRecord / CapabilitySourceProfileRecord / CapabilityDonorTrustRecord` 以及 `CapabilityDonorService / CapabilityPortfolioService`，candidate 落库时会同步归一 donor/package/source truth。Runtime Center 现已新增 `/runtime-center/capabilities/donors` 与 `/runtime-center/capabilities/source-profiles` 读面，`governance/status` 和 `capability-optimizations` 也开始返回正式 `portfolio` 诊断，包含 `donor_count / trusted_source_count / watchlist_source_count / degraded_donor_count / retire_pressure_count / over_budget_scope_count / planning_actions`，并把 `donor-trust / portfolio-density` 纳入 degraded components。当前定向验证已通过：`$env:PYTHONPATH='src'; C:\\Python312\\python.exe -m pytest tests/predictions/test_skill_candidate_service.py tests/app/test_runtime_center_events_api.py tests/app/runtime_center_api_parts/overview_governance.py tests/predictions/test_skill_trial_service.py tests/app/test_runtime_bootstrap_helpers.py tests/app/test_system_api.py tests/app/test_capability_market_api.py tests/app/test_runtime_canonical_flow_e2e.py tests/app/test_operator_runtime_e2e.py -q`，合计 `172 passed`。这条记录表示“完整任务主链稳定闭环”“长期无人值守 runtime”在当前验证口径下保持稳定，同时 donor/source/portfolio 的 Runtime Center 读面与 portfolio-oriented planning 已进入正式主链。
- `2026-04-04` 收口补充：donor-first 第二波 `P1/P2` 剩余 repo gap 现已一次性压实，不再留“正式 lifecycle 已有，但 actor/industry/prediction 入口还在走旧 assignment 语义”这类尾巴。`runtime_center_actor_capabilities.py`、`industry/service_team_runtime.py`、`predictions/service_recommendations.py` 现已统一改走 `system:apply_capability_lifecycle`，actor/runtime-center/industry bootstrap 的 capability assignment 不再继续生成新的 `system:apply_role` mutation 任务。与此同时，`RuntimeCenterStateQueryService` 与 `PredictionService` 自己复制的一套 portfolio/discovery 统计逻辑已删除，正式统一委托 `CapabilityPortfolioService`；runtime discovery posture 现统一暴露 `active_source_count / trusted_source_count / watchlist_source_count / fallback_only_source_count / trust_posture_count / degraded_components`，skill/package metadata discipline 也正式落到 `package_kind_count / candidate_source_kind_count / target_scope_count / scope_breakdown` 这条单一 read-model 上。对应 focused verification：`$env:PYTHONPATH='src'; C:\\Python312\\python.exe -m pytest tests/app/test_runtime_center_actor_api.py tests/predictions/test_skill_candidate_service.py tests/app/test_predictions_api.py tests/app/test_runtime_center_events_api.py -q` -> `45 passed`；相邻主链回归：`tests/app/test_capability_market_api.py` -> `27 passed`、`tests/app/industry_api_parts/runtime_updates.py` -> `40 passed`、`tests/app/industry_api_parts/bootstrap_lifecycle.py` -> `34 passed`、`tests/app/runtime_center_api_parts/overview_governance.py` -> `81 passed`、`tests/kernel/test_query_execution_runtime.py` -> `21 passed`。这表示 donor-first 当前定义下的 `P0 + P1/P2` 已在主线代码口径上完成闭环；后续若继续推进，应进入新的优化波次，而不是继续补这轮遗留。
- `2026-04-04` 实施补充：第一波 donor-first 外扩 + `P0(A/B/C)` 已继续从“对象和读面”推进到“正式写链 + discovery spine + entropy contract + runtime governance integration”的同波实现。当前仓库已新增 `src/copaw/discovery/models.py`、`src/copaw/discovery/source_chain.py`、`src/copaw/discovery/deduplication.py` 与 `src/copaw/state/donor_source_service.py`，正式支持 `primary -> mirror -> fallback` 单次单活 source-chain、last-known-good snapshot fallback、以及 discovery 命中在 candidate fan-out 前的多源去重/归一。与此同时，`CapabilityCandidateService` 已能导入 normalized discovery hits，并把 `canonical_package_id / source_aliases / equivalence_class / capability_overlap_score / replacement_relation` 落到正式 candidate truth；`SkillTrialService` 与 `SkillLifecycleDecisionService` 也已接上同一批 attribution/retirement 字段，不再只保留薄的 `candidate_id` 写链。`query_execution_runtime` 与 `conversation_compaction_service` 现已补上 donor/trial-heavy turn 的 entropy contract：显式 tool-result budget、bounded carry-forward、operator-visible donor-trial carry-forward degradation。`Runtime Center` 也新增了 `/runtime-center/capabilities/portfolio` 与 `/runtime-center/capabilities/discovery`，capability governance 读面现在会统一投影 `portfolio + discovery`，并把 fallback-only/local-authored inventory 从 governed donor portfolio/discovery 计数里剥离。fresh verification：`$env:PYTHONPATH='src'; C:\\Python312\\python.exe -m pytest tests/state/test_capability_donor_truth.py tests/state/test_donor_source_service.py tests/discovery/test_source_chain.py tests/discovery/test_deduplication.py tests/predictions/test_skill_candidate_service.py tests/predictions/test_skill_trial_service.py tests/app/test_predictions_api.py tests/kernel/test_query_execution_runtime.py tests/app/test_runtime_center_events_api.py tests/app/runtime_center_api_parts/overview_governance.py tests/app/test_runtime_bootstrap_helpers.py -q` -> `176 passed`；邻接长链回归：`$env:PYTHONPATH='src'; C:\\Python312\\python.exe -m pytest tests/app/test_runtime_canonical_flow_e2e.py tests/app/test_operator_runtime_e2e.py tests/app/test_system_api.py tests/app/test_capability_market_api.py tests/app/test_predictions_api.py -q` -> `64 passed`。当前残余 follow-up 主要落在 `P1/P2`：writer contract repo-wide adoption、unified child-run shell、MCP lifecycle hardening、skill/package metadata tightening、portfolio hotspot cooling。
...
- `2026-04-04` donor-first 外扩剩余闭环现已一次性收口，不再只停在 donor/source/portfolio 的第一层 truth。当前仓库已补上 `src/copaw/discovery/opportunity_radar.py` 与 `src/copaw/discovery/scout_service.py`，正式提供 bounded `opportunity` radar 与自治 `scout` 入口；同时新增 `src/copaw/state/donor_package_service.py`、`src/copaw/state/donor_trust_service.py`，把 package summary、trial/rollback/retirement 聚合 trust memory 从 `CapabilityDonorService` 的底层 truth 中抽成正式服务层。runtime bootstrap/query 也已继续接线：`RuntimeBootstrap` 现显式携带 `donor_source_service / donor_package_service / donor_trust_service / donor_scout_service`，`RuntimeCenterStateQueryService` 与 `/runtime-center/capabilities/*` 已新增 `packages / trust / scout` 读面，不再只有 donors/source-profiles/portfolio/discovery。prediction 侧已补上 donor recommendation 排序纪律：external donor + replacement-first 优先，`local_authored` 明确退到 last-resort。fresh verification：`$env:PYTHONPATH='src'; C:\\Python312\\python.exe -m pytest tests/state/test_donor_source_service.py tests/state/test_donor_package_service.py tests/state/test_donor_trust_service.py tests/discovery/test_source_chain.py tests/discovery/test_opportunity_radar.py tests/discovery/test_deduplication.py tests/discovery/test_scout_service.py tests/predictions/test_donor_recommendations.py tests/app/test_runtime_center_donor_api.py tests/kernel/test_query_execution_runtime.py -q` -> `33 passed`；相邻回归：`$env:PYTHONPATH='src'; C:\\Python312\\python.exe -m pytest tests/predictions/test_skill_candidate_service.py tests/predictions/test_skill_trial_service.py tests/app/test_capability_market_api.py tests/app/test_predictions_api.py tests/app/test_runtime_center_events_api.py tests/app/test_runtime_bootstrap_helpers.py -q` -> `94 passed`。这条记录表示 `2026-04-04-external-capability-assimilation-implementation-plan.md` 里剩余未落地的 `opportunity radar / autonomous scout / donor package-trust services / runtime-center donor API` 已进入主线代码基线。
- `2026-04-04` donor-first capability evolution priority plan 的 `Task 1 ~ Task 4` 现已按 donor-first 口径落到主线，不再停在“外扩基座已完成，但 reuse/trial/runtime-center 仍缺胶水”的状态。当前仓库新增了 `src/copaw/capabilities/skill_evolution_service.py` 与 `src/copaw/learning/skill_gap_detector.py`：前者把 `reuse_existing_candidate / adopt_registered_package / adopt_external_donor / author_local_fallback` 收成单一 resolver，明确 `healthy reuse -> registered package -> external donor -> local fallback` 的决策顺序，并支持 `mcp-bundle` 这类非 skill-only candidate form；后者把 repeated failure / operator takeover / rollback pressure 统一投影成 formal drift re-entry summary。与此同时，`CapabilitySkillService` 已只在 resolver 明确要求时才 materialize fallback local artifact，`SkillTrialService` 已提供 candidate-level verdict aggregation，`query_execution_runtime` 与 `query_execution_context_runtime` 会继续携带 `donor_id / package_id / source_profile_id / candidate_source_kind / resolution_kind` 等 attribution 到 evidence sink/runtime metadata，`industry/service_runtime_views.py` 与 `RuntimeCenterStateQueryService` 也开始稳定暴露 `current_capability_trial / supply_path / provenance / lifecycle_history / drift_reentry`，不再需要从分散 metadata 猜测来源。当前 focused verification：`python -m pytest tests/test_skill_service.py tests/app/test_capability_skill_service.py tests/predictions/test_skill_trial_service.py tests/kernel/test_query_execution_runtime.py tests/app/test_runtime_center_events_api.py tests/app/test_capability_market_api.py tests/industry/test_runtime_views_split.py -q` -> `98 passed`；相邻回归：`python -m pytest tests/test_mcp_resilience.py tests/app/test_mcp_runtime_contract.py -q` -> `23 passed`，`python -m pytest tests/predictions/test_skill_candidate_service.py -q` -> `6 passed`。更大的 `tests/app/industry_api_parts/bootstrap_lifecycle.py` 与 `tests/app/test_industry_api.py` 在当前 shell 超时预算内未跑完整 summary，但超时前未出现 failure，分别已连续通过 `23/34` 与 `70+/111` 个用例；后续如需要，可在更长超时或分片模式下继续补整套 summary。
- 当前产品口径已经固定为：
  - 人类默认先和主脑说话
  - 主脑决定本轮是继续聊天还是进入执行编排
  - 高风险动作仍回到正式治理链
