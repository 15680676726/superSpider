...
9. `docs/superpowers/specs/2026-04-01-formal-planning-capability-gap-design.md`（如任务涉及短中长期 formal planner 缺口、planning shell external-source 边界、assignment/cycle/strategy planning 能力建设）
...
19. `docs/superpowers/specs/2026-04-04-external-capability-assimilation-architecture.md`（如任务涉及 external-source-first 能力吸纳、外部项目/skill/MCP/adapter/provider 供给面、source chain、opportunity radar、多源去重与能力组合治理）
20. `docs/superpowers/plans/2026-04-04-external-capability-assimilation-implementation-plan.md`（如任务涉及 external-source-first 外部能力吸纳的对象落地、discovery source chain、自主 scout、portfolio governance 与 Runtime Center 接线）
...
22. `docs/superpowers/plans/2026-04-04-external-source-capability-evolution-priority-plan.md`（如任务涉及纠偏后的 capability evolution 下一轮施工顺序、external-source-first 复用优先级、baseline import、MCP-native candidate、fallback-only local authored 边界）
23. `docs/superpowers/specs/2026-04-06-universal-external-execution-contract-design.md`（如任务涉及 external-source formal provider injection、universal execution envelope、host compatibility contract、真实 external-source “安装后不等于真正可用”的平台收口）
...
- 旧 GitHub/open-source project source 接入面不再以“任意项目接入”为正式目标，后续要收口成“只接执行体 runtime provider”
...
- 当前还没彻底收口的 external-source-first 遗留包括：
  - `/capability-market/projects/install*`
  - `project-package / adapter / runtime-component`
  - external-source trust/trial/portfolio state
  - Runtime Center external-source/external-runtime 双读面
  - external-source-first 旧 specs / tests / TASK_STATUS 口径
...
- 当前能诚实表述的结论：
  ...
  - 这不等于整个 external-executor hard-cut 已完成，更不等于 `delegation_service.py` / external-provider 边界已全部删除
...
- 当前仍然明确未完成：
  ...
  - `/capability-market/projects/install*` / external-provider compatibility 边界未在本轮继续推进
...
## 1.0.5 `2026-04-21` external-source/project intake compatibility-acquisition-only 标记收口

- 当前工作树继续把 `Task 4` 往前推了一个正式表面降级切片，但这次仍没有把 external-source/project intake 替换成 formal executor provider：
  - `src/copaw/capabilities/project_donor_contracts.py`
    - 已给 external project source contract metadata 与 projected package metadata 统一补上：
  ...
  - `src/copaw/app/routers/capability_market.py`
    ...
    - 上述 external-source/project surface 现都会显式返回 compatibility/acquisition-only 标记，不再裸露成像 formal executor-provider intake 的产品面
    - project candidate materialization / install contract metadata / persisted external package metadata 也会继承同一标记
  - `src/copaw/app/runtime_center/state_query.py`
    - Runtime Center external-source/package projection 现统一补上 compatibility/acquisition-only 标记
    - external-source/package 读面同时兼容 dict-backed service payload，不再只接受 `model_dump()` 结果
- 当前能诚实表述的结论：
  - external-source/project intake 这条产品与状态读面现在已明确标为 `compatibility/acquisition-only`
  - 这证明 `Task 4` 的“先标记 compatibility，再阻止误读为 canonical executor surface”切片已到 `L1 + L2`
  - 这不等于 formal `ExecutorProvider` intake 已落地，更不等于 external-source/project runtime taxonomy 已从所有 state/trial/read 面彻底拆除
- 当前 fresh focused regression 证据：
  - external-source/project compatibility surface focused regression：
    ...
    - 验收层级：`L1 + L2`
- 当前仍然明确未完成：
  - formal `ExecutorProvider / control_surface_kind / protocol_surface_kind` intake 仍未完全替换 external-source/project install 前门
  - external-source state/trust/trial/retirement 全量 taxonomy 仍未完成正式拆分
  - 共享 `default regression` / selected `L3` / selected `L4` 证据现已补齐，详见 `1.0.6`
  - 因此 external-source/project compatibility demotion 这条 slice 现在只能诚实写成 `L1 + L2 + default regression + selected L3 + selected L4`
...
- 当前 fresh verification 证据：
  ...
  - `L4` long soak：
    ...
    - 验收层级：`selected L4`
- 当前能诚实表述的结论：
  - actor-runtime `read-only-compat`、delegation default-surface demotion、以及 external-source/project compatibility demotion 这三个 `2026-04-21` 收口 slice 现在都已具备 `L1 + L2 + default regression + selected L3 + selected L4`
...
- 当前能诚实表述的结论：
  ...
  - formal `ExecutorProvider` intake 现在已经存在，external-source/project install 不再是唯一也不再是假装 canonical 的 executor intake 前门
  - external-source/project compatibility/acquisition-only 产品壳仍然存在，因此这不等于 external-source taxonomy / external-source state/trust/trial/retirement 已全部删除
- 当前 fresh focused regression 证据：
  - focused regression：
...
- 当前仍然明确未完成：
  ...
  - external-source/project state/trust/trial/retirement 全量 taxonomy 仍未全部退役；`/capability-market/projects/install*` 仍然保留 compatibility/acquisition-only 前门
...
- 本轮按 `docs/superpowers/plans/2026-04-21-external-executor-hard-cut-remaining-work-plan.md` 连续完成并提交了 Task 1-6：
  ...
  - `Task 5 / external-source execution shell acquisition-only`
    - `2d38c65`：external-source/project metadata、Capability Market project install `runtime_contract`、Runtime Center external-source projection 的 nested execution shell 现统一补 `compatibility/acquisition-only`
...
- 当前能诚实表述的结论：
  ...
  - external-source/project 与 delegation child-run 现都只以显式 compatibility surface 存在，不再裸露成 formal executor intake / formal assignment child backend
...
- 当前仍然明确未完成：
  ...
  - `delegation_service.py`、actor runtime 代码与 external-source/project taxonomy 仍保留 compatibility 遗留，尚未物理删除
  - `/capability-market/projects/install*` compatibility/acquisition-only 前门仍在，不得误写成 external-source-first 产品壳已删除
...
- 当前能诚实写出的结论：
  ...
  - actor runtime、`delegation_service.py`、external-source/project product shell 仍保留 compatibility/deprecation 代码，但它们已经不再处在 canonical formal execution path 上；因此它们现在是 **后续删除项**，不是 external-executor hard-cut 终态完成的阻塞项
- fresh 验收证据：
  - focused regression：
...
## 1.2 `2026-04-05` external-source-first 开源项目能力落位补充

- GitHub open-source external-source 不再只以 `SKILL.md` bundle 形式落地。
- 正式新增的一等 capability landing：
  - `project-package`
  - `adapter`
  - `runtime-component`
- `/capability-market/projects/install` 已改为真实开源项目 materialization 前门，不再先降成 `skill:*` 再冒充 external project source。
- config-backed external capability mounts 已接入统一 `capability graph / catalog / execution`，可以被正式列出、切换、删除、执行。
- 当前默认 external project source materialization 已改成 bounded HTTP transport first：优先 `codeload tar.gz -> GitHub archive zip`，`git clone` 只作为最后兜底，避免真实网络环境下先卡死在长时间 `git clone`。
- `2026-04-05` fresh regression：
  - `129 passed`
- `2026-04-05` live smoke 已真实验证：
  - GitHub external-source search：
    - `psf/black`
    - `HKUDS/OpenSpace`
...
- 当前 external-source Python runtime 的真实边界：
  - CLI runtime source 闭环已被真实验证
  - service runtime source 的 formal install/start/healthcheck/stop/state chain 已被真实验证
  - 至少 1 个真实 Python service runtime source（`HKUDS/OpenSpace`）已完成 `search -> install -> ready -> stop`
  - 当前边界仍是“符合 Python/GitHub/isolated-venv/runtime-contract 合同的项目”，不等于任何语言、任何构建系统的任意仓库都已自动适配

## 1.2.3 `2026-04-06` external-source install async job front-door 补充
...
- 当前正式前门合同改为：
  ...
  - `GET /capability-market/projects/install-jobs/{task_id}/result`
    - 仅在 job `completed` 后返回最终 external-source install 结果
...
- 这样重 external-source（例如此前会把前端长请求顶超时的 service runtime source）不再依赖单个 HTTP 请求硬撑到安装完成。
...
- `2026-04-06` fresh verification 补充：
  - 直接 external-source install 内核 smoke：
...
- 当前诚实边界：
  ...
  - 但真实大型 external-source 的单阶段耗时仍可能主要集中在 `pip_install`
  - 因此这轮解决的是“别再假卡死、别再靠长 HTTP 顶住、别再没有 terminal truth”，不是“所有 external-source 安装都会很快”

## 1.2.1 `2026-04-06` external-source adapter common-base assimilation 补充
...
- 当前 fresh regression：
  ...
  - 结果：`148 passed`
- 当前已验证事实：
  - runtime-only external source 现在会被正式阻断，不能再冒充 business adapter
...
- 当前诚实边界：
  ...
  - 对 `adapter` 而言，公共执行/治理主链已完成；但“从任意真实 external-source 自动抽取 `mcp_tools / api_actions / sdk_actions`”仍未作为通用 live-verified discovery 合同落地

## 1.2.2 `2026-04-06` universal external execution contract 设计补充

- 已新增正式设计文档：
  - `docs/superpowers/specs/2026-04-06-universal-external-execution-contract-design.md`
- 该设计不是 `OpenSpace` 单项目修补，而是把真实 live probe 暴露出来的 3 个平台缺口上升成通用 external-source 合同：
  - `provider injection`
  - `execution envelope / fail-fast`
  - `host compatibility contract`
- 该设计的核心口径是：
  - external-source 可以有不同 transport / packaging / startup 方式
...
- `2026-04-06` 当前实现状态补正：
  - `src/copaw/capabilities/donor_probe_service.py` 已补齐最小 probe 正式服务：
    ...
    - 无 formal probe 路径的 external-source 保持在 `installed`，不会被夸大
  ...
  - Runtime Center external-source 读面已把 `metadata.probe_result` 显式投影成 top-level：
    - `selected_adapter_action_id`
    - `probe_outcome`
    - `probe_error_type`
    - `probe_evidence_refs`
  - `query_execution_runtime` 已把 external-source probe attribution 正式带入 evidence metadata：
    - `verified_stage`
    ...
    - `probe_error_type`
    - `probe_evidence_refs`
  - 当前 fresh external-source verification：
    ...
    - 结果：`113 passed`
- 当前诚实边界：
  - 这轮完成的是 universal external execution contract 的 Task 5/6 落地和读面/evidence 收口
  - 不等于“任意 external-source 都能自动发现 typed business action 并完整业务可用”
  - `project-package / runtime-component / adapter` 的 formal execution/probe/promotion 主链已接通，但更强的 external-source action discovery 与更大范围 live external-source 适配仍是后续增强项
...
### 3.3.1 2026-04-06 同日 partial 记录说明

上面这批同日 partial 审计原本用于记录当日中途核查重点；此前文档批量清理时该段发生错误转码，现收敛为可公开摘要。

当日 partial 核查的三条主线是：

- industry kickoff / auto-resume 与 legacy goal/schedule 的残留耦合
  - 重点关注 kickoff_execution_from_chat(...)、_should_auto_resume_execution_stage(...)、_reconcile_kickoff_autonomy_status(...) 与 service_activation.py
  - 重点判断 execution-stage admission truth 是否已经切离 legacy goal/schedule，以及 bootstrap auto-learning / kickoff 是否仍会回到旧 dispatch 语义
- acquisition producer / kernel identity 的收口状态
  - 重点关注 learning/acquisition_runtime.py
  - 重点判断 free-form DecisionRequestRecord / TaskRecord producer fallback 是否仍能绕开 kernel dispatcher-backed task store
- capability execution 前门是否已经统一进入 kernel / governed mutation
  - 重点关注 query-time builtin tool delegate、eact_agent builtin fallback、system_dispatch
  - 重点判断 child kernel task submit -> execute_task、failure status 与 kernel admission 是否仍存在 legacy shortcut

当日 focused / file-level verification 包括：
...
- 同日最终判断以后续 3.3.2 权威更新为准
- 更晚的 focused harness、删旧进度与 live smoke 结论以后续更新段为准
