...
- `2026-04-16` 补充：MCP 自动激活 / 自动自愈的第一刀已按正式边界落到 `browser-companion` install-template surface，而不是继续停留在“装了 capability 但还得手工 attach”这类半成品口径。当前 landed slice 只做一件事：`/capability-market/install-templates/browser-companion/example-run` 在发现“无 mounted session”或“已有 canonical session 但 browser companion truth 缺失/不可用”时，会先走同一条正式主链做 bounded remediation：1) 复用或创建 canonical `SessionMount` 浏览器租约；2) 启动 managed browser runtime；3) 通过 `EnvironmentService` 正式写入 browser companion / browser attach truth；4) 重新读取同一条 canonical runtime projection，再决定 success/error，不再直接把 `runtime-unavailable` / `adapter-unavailable` 生硬暴露给用户。当前边界也已写死：doctor 仍保持 diagnostic-only，这一刀还没有把整套 `stateless / auth-bound / host-attached / workspace-bound` MCP lifecycle 全部通用化。相关设计/计划文档已新增 `docs/superpowers/specs/2026-04-16-mcp-activation-self-heal-design.md` 与 `docs/superpowers/plans/2026-04-16-mcp-activation-self-heal-implementation-plan.md`；focused verification：`python -m pytest tests/app/test_capability_market_api.py -q -k "browser_companion_example_run_auto_activates_managed_browser_when_unmounted or browser_companion_example_run_repairs_existing_mounted_session"` -> `2 passed`；adjacent regressions：`python -m pytest tests/environments/test_cooperative_browser_companion.py -q` -> `15 passed`；`python -m pytest tests/app/test_capability_market_api.py -q -k "browser_companion or browser_local_session_start"` -> `6 passed`；`python -m pytest tests/app/test_runtime_bootstrap_split.py -q -k "environment_service_rebinds_cooperative_facade_after_late_bootstrap_injection or browser_local_session_start_resolves_to_attached_browser_channel_when_available"` -> `1 passed`。
- `2026-04-16` 设计补充：围绕上面这一刀，统一泛化方案也已正式收口，不再继续靠会话上下文记忆。仓库已新增 `docs/superpowers/specs/2026-04-16-unified-mcp-activation-lifecycle-design.md` 与 `docs/superpowers/plans/2026-04-16-unified-mcp-activation-lifecycle-implementation-plan.md`，把后续施工固定为：1) 共享 activation status/reason/result contract；2) 共享 activation/self-heal orchestrator；3) 保持 `doctor` read-only，并新增显式 `activate` 作为 bounded write surface；4) `host-attached` 四个 cooperative templates 先并入同一 plane；5) 再补 `workspace-bound` overlay lifecycle；6) 再补 `auth-bound` 人工边界；7) 最后补 `stateless` 并统一 Capability Market/Runtime Center 读面；同时让真实 capability execution front-door 也先走同一 activation plane，而不是要求用户手工先跑 doctor/example-run。该设计明确锁死边界：activation plane 是对既有 `CapabilityMount / EnvironmentMount / SessionMount / MCPClientRuntimeRecord` 的 derived execution orchestration，不新增第二 durable activation store，也不把 `MCPClientManager` 或 `EnvironmentService` 重新做成平行产品子系统。
