...
- `2026-04-05` donor live gap closure：上述 4 个 live gap 已在主线代码补齐。`source_chain.execute_discovery_action()` 现已把 `0 hit` 收口为 `empty`，会继续尝试下一个 source，且不会把空结果写成 last-known-good success；`runtime_service_graph._execute_runtime_discovery_action()` 已从空实现改成真实 provider dispatcher，当前默认支持 `SkillHub / GitHub repo search / official MCP registry`；`runtime_service_graph` 的 `OpportunityRadarService` 也不再默认 `feeds={}`，而是带 bounded `github-trending / mcp-registry` feeds；`src/copaw/discovery/provider_search.py` 新增了 GitHub donor search 正式适配层；`search_hub_skills()`、`search_curated_skill_catalog()` 现会做 SkillHub bundle installability 校验并抑制坏包。当前 focused regression：`PYTHONPATH=src python -m pytest tests/state/test_donor_source_service.py tests/discovery/test_source_chain.py tests/discovery/test_scout_service.py tests/discovery/test_opportunity_radar.py tests/discovery/test_provider_search.py tests/agents/test_skills_hub.py tests/app/test_runtime_bootstrap_helpers.py tests/app/test_runtime_center_donor_api.py tests/app/test_capability_market_api.py -q` -> `73 passed`；donor read-model 相邻回归：`PYTHONPATH=src python -m pytest tests/app/test_runtime_center_events_api.py tests/app/runtime_center_api_parts/overview_governance.py tests/predictions/test_donor_recommendations.py tests/predictions/test_skill_candidate_service.py tests/predictions/test_skill_trial_service.py -q` -> `107 passed`。真实联网验收也已补上：1) `/capability-market/hub/search?q=agent browser` 实测只返回可安装包，`agent-browser` 安装成功；2) `/capability-market/curated-catalog?q=agent browser` 会显式报告已抑制的坏 bundle 数，并成功安装 `agent-browser`；3) `/capability-market/mcp/catalog?q=filesystem` + detail/install 实测成功安装 `io.github.Digital-Defiance/mcp-filesystem`，随后 `MCPClientManager.init_from_config()` 返回 `status=ready`；4) `DonorScoutService` 现已能在真实 runtime executor 上通过 `browser automation github` 和 `opportunity` 模式导入真实 GitHub donor candidates，不再默认 `0 imported candidates`。这条记录之后，donor-first 这轮“真实外扩发现/安装/使用”主题不再有已知 live 缺口。
- 当前产品口径已经固定为：
  - 人类默认先和主脑说话
  - 主脑决定本轮是继续聊天还是进入执行编排
  - 高风险动作仍回到正式治理链
