...
- `2026-04-02` 补充：`cc/cowork` donor 对齐又推进了一轮真实主链落地，而不只是文档映射。`QueryExecutionRuntime` 现已在进入长执行前记录正式 `accepted` persistence boundary，并把 `accepted_persistence / commit_outcome` 写回 runtime context；runtime chat sidecar 不会再在已知 writeback 失败时继续乐观发 `committed`。同时，`Runtime Center` 已新增正式 bridge lifecycle ops：`ack / heartbeat / stop / reconnect / archive / deregister` 可直接驱动同一条 `EnvironmentMount / SessionMount` 真相链，`health_service` 也已把 `bridge_work_id / bridge_work_status / bridge_heartbeat_at / bridge_session_id / bridge_stopped_at / bridge_stop_mode / workspace_trusted / elevated_auth_state` 投影进 `seat_runtime / host_companion_session.bridge_registration`。另外，desktop app projection 现已对 prompt-facing `app_identity / window_anchor_summary` 做 sanitization，避免宿主侧恶意字符串原样进入 execution/runtime 读面；但 execution-time `ESC abort / host exclusion / clipboard verify / frontmost gate` 等 computer-use guardrail 仍未完成。
