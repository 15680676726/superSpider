...
$ProductDisplayName = "Spider Mesh"
$InstallerNamePrefix = "Spider-Mesh-Setup"
$LauncherBaseName = "baize-desktop"
$DebugLauncherBaseName = "baize-desktop-debug"
...
$SkipWheelBuild = $env:SKIP_WHEEL_BUILD -eq "1"
if ($SkipWheelBuild) {
  Write-Host "SKIP_WHEEL_BUILD=1 set, reusing existing dist wheel."
} else {
...
$LauncherBat = Join-Path $EnvRoot "$LauncherBaseName.bat"
@"
@echo off
cd /d "%~dp0"
...
$DebugBat = Join-Path $EnvRoot "$DebugLauncherBaseName.bat"
@"
@echo off
chcp 65001 >nul
cd /d "%~dp0"
echo ====================================
echo Spider Mesh - Debug Mode
echo ====================================
...
echo [Launch] 正在启动 Spider Mesh...
echo Press Ctrl+C to stop
echo.
"%~dp0python.exe" -m copaw desktop
echo.
echo [Exit] Spider Mesh 已关闭
pause
"@ | Set-Content -Path $DebugBat -Encoding UTF8

# VBScript launcher (no console window)
$LauncherVbs = Join-Path $EnvRoot "$LauncherBaseName.vbs"
@"
Set WshShell = CreateObject("WScript.Shell")
batPath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\$LauncherBaseName.bat"
...
$OutInstaller = Join-Path (Join-Path $RepoRoot $Dist) "$InstallerNamePrefix-$Version.exe"
