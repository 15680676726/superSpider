#!/usr/bin/env bash
# One-click build: console -> conda-pack -> Spider Mesh.app. Run from repo root.
...
APP_NAME="Spider Mesh"
APP_EXECUTABLE="baize-desktop"
...
if [[ "${SKIP_WHEEL_BUILD:-0}" == "1" ]]; then
  echo "SKIP_WHEEL_BUILD=1 set, reusing existing dist wheel."
else
...
cat > "${APP_DIR}/Contents/MacOS/${APP_EXECUTABLE}" << 'LAUNCHER'
#!/usr/bin/env bash
...
if [ ! -t 2 ]; then
  mkdir -p "$HOME/.copaw"
  { echo "=== $(date) Spider Mesh starting ==="
...
<dict>
  ...
  <key>CFBundleIdentifier</key><string>com.baize.desktop</string>
  <key>CFBundleName</key><string>${APP_NAME}</string>
  <key>CFBundleVersion</key><string>${VERSION}</string>
  <key>CFBundleShortVersionString</key><string>${VERSION}</string>
  ${ICON_PLIST}<key>NSHighResolutionCapable</key><true/>
  <key>LSMinimumSystemVersion</key><string>14.0</string>
  <key>NSDesktopFolderUsageDescription</key><string>Spider Mesh 在你使用文件相关功能时可能访问桌面中的文件。你也可以选择“不允许”，系统仍可运行，但文件能力会受限。</string>
</dict>
</plist>
INFOPLIST
...
if [[ -n "${CREATE_ZIP}" ]]; then
  ZIP_NAME="${DIST}/Spider-Mesh-${VERSION}-macOS.zip"
