...
function Install-QmdSidecar {
    if (Get-Command qmd -ErrorAction SilentlyContinue) {
        Write-Info "QMD sidecar already installed"
        return
    }
    if (-not (Get-Command npm -ErrorAction SilentlyContinue)) {
        Write-Warn "npm not found - skipping built-in QMD install."
        Write-Warn "Install Node.js from https://nodejs.org/ then run: npm install -g @tobilu/qmd"
        return
    }
    Write-Info "Installing built-in QMD sidecar (npm install -g @tobilu/qmd)..."
    npm install -g @tobilu/qmd
    if ($LASTEXITCODE -ne 0) {
        Write-Warn "QMD install failed. CoPaw will keep running with local memory backends."
        return
    }
    if (Get-Command qmd -ErrorAction SilentlyContinue) {
        Write-Info "QMD sidecar installed successfully"
        return
    }
    Write-Warn "QMD install command finished but qmd was not found on PATH."
}
