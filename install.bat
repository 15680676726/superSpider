...
where qmd >nul 2>&1
if not errorlevel 1 (
    echo [copaw] QMD sidecar already installed
    exit /b 0
)
where npm >nul 2>&1
if errorlevel 1 (
    echo [copaw] WARNING: npm not found - skipping built-in QMD install.
    echo [copaw] WARNING: Install Node.js from https://nodejs.org/ then run: npm install -g @tobilu/qmd
    exit /b 0
)
echo [copaw] Installing built-in QMD sidecar (npm install -g @tobilu/qmd)...
npm install -g @tobilu/qmd
if errorlevel 1 (
    echo [copaw] WARNING: QMD install failed. CoPaw will keep running with local memory backends.
    exit /b 0
)
where qmd >nul 2>&1
if errorlevel 1 (
    echo [copaw] WARNING: QMD install finished but qmd was not found on PATH.
    exit /b 0
)
echo [copaw] QMD sidecar installed successfully
exit /b 0
