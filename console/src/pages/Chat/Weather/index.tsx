import { useMemo } from "react";
import { Card, Typography } from "antd";
import { createStyles } from "antd-style";
import dayjs from "dayjs";
import { Cloud, CloudRain, Sun } from "lucide-react";

interface WeatherItem {
  location: string;
  weather: string;
  temperature: number;
  date: string;
}

const useStyles = createStyles(({ css }) => ({
  container: css`
    width: 100%;
    max-width: 320px;
    border: none;
    border-radius: 20px;
    overflow: hidden;
    color: #fff;
    background: linear-gradient(135deg, #4f7cff 0%, #1138c2 100%);
    box-shadow: 0 10px 20px rgba(0, 0, 0, 0.15);

    .ant-card-body {
      padding: 0;
    }
  `,
  header: css`
    display: flex;
    justify-content: space-between;
    align-items: flex-start;
  `,
  location: css`
    color: white !important;
    font-size: 24px;
    font-weight: 600;
    margin-bottom: 4px;
  `,
  date: css`
    color: white !important;
    font-size: 14px;
    opacity: 0.85;
  `,
  mainWeather: css`
    display: flex;
    flex-direction: column;
    align-items: center;
    padding: 24px 0 32px;
  `,
  tempContainer: css`
    display: flex;
    align-items: flex-start;
    line-height: 1;
  `,
  temperature: css`
    color: white !important;
    font-size: 64px;
    font-weight: 700;
  `,
  degree: css`
    color: white !important;
    font-size: 24px;
    font-weight: 500;
    margin-top: 8px;
  `,
  mainIcon: css`
    font-size: 48px;
    margin-bottom: 16px;
    filter: drop-shadow(0 4px 4px rgba(0, 0, 0, 0.2));
  `,
  condition: css`
    color: white !important;
    font-size: 16px;
    font-weight: 500;
    margin-top: 8px;
    opacity: 0.9;
  `,
  forecast: css`
    padding: 16px 24px;
    background: rgba(255, 255, 255, 0.1);
    backdrop-filter: blur(10px);
  `,
  forecastItem: css`
    display: flex;
    justify-content: space-between;
    align-items: center;
    padding: 12px 0;
    border-bottom: 1px solid rgba(255, 255, 255, 0.1);

    &:last-child {
      border-bottom: none;
      padding-bottom: 4px;
    }

    &:first-of-type {
      padding-top: 4px;
    }
  `,
  forecastDay: css`
    color: white !important;
    width: 72px;
    font-size: 14px;
  `,
  forecastIcon: css`
    color: white;
    font-size: 20px;
  `,
  forecastTemp: css`
    color: white !important;
    width: 40px;
    font-size: 14px;
    font-weight: 500;
    text-align: right;
  `,
}));

function WeatherIcon({
  type,
  className,
}: {
  type: string;
  className?: string;
}) {
  switch (type) {
    case "rainy":
      return <CloudRain className={className} />;
    case "cloudy":
      return <Cloud className={className} />;
    case "sunny":
    default:
      return <Sun className={className} />;
  }
}

function getWeatherLabel(type: string): string {
  switch (type) {
    case "sunny":
      return "晴朗";
    case "rainy":
      return "雨天";
    case "cloudy":
      return "多云";
    default:
      return type;
  }
}

function isWeatherItem(value: unknown): value is WeatherItem {
  if (!value || typeof value !== "object") {
    return false;
  }
  const item = value as Partial<WeatherItem>;
  return (
    typeof item.location === "string" &&
    typeof item.weather === "string" &&
    typeof item.temperature === "number" &&
    typeof item.date === "string"
  );
}

function parseWeatherOutput(output: string): WeatherItem[] {
  let parsed: unknown = output;

  for (let depth = 0; depth < 2; depth += 1) {
    if (typeof parsed !== "string") {
      break;
    }
    try {
      parsed = JSON.parse(parsed);
    } catch {
      return [];
    }
  }

  if (!Array.isArray(parsed)) {
    return [];
  }

  return parsed.filter(isWeatherItem);
}

function formatForecastDate(date: string, index: number): string {
  if (index === 0) {
    return "今天";
  }
  return dayjs(date).format("MM/DD");
}

export default function Weather(props: {
  data: { content: Array<{ data: { output: string } }> };
}) {
  const { styles } = useStyles();
  const data = useMemo(() => {
    const content = props.data?.content;
    if (!content || content.length < 2) {
      return [];
    }
    const output = content[1]?.data?.output;
    if (!output) {
      return [];
    }
    return parseWeatherOutput(output);
  }, [props.data.content]);

  if (data.length === 0) {
    return null;
  }

  const current = data[0];
  const forecast = data.slice(1);

  return (
    <Card className={styles.container} bordered={false}>
      <div className={styles.header}>
        <div>
          <Typography.Text className={styles.location}>
            {current.location}
          </Typography.Text>
          <br />
          <Typography.Text className={styles.date}>
            {dayjs(current.date).format("MM月D日")}
          </Typography.Text>
        </div>
      </div>

      <div className={styles.mainWeather}>
        <WeatherIcon type={current.weather} className={styles.mainIcon} />
        <div className={styles.tempContainer}>
          <Typography.Text className={styles.temperature}>
            {current.temperature}
          </Typography.Text>
          <Typography.Text className={styles.degree}>°C</Typography.Text>
        </div>
        <Typography.Text className={styles.condition}>
          {getWeatherLabel(current.weather)}
        </Typography.Text>
      </div>

      <div className={styles.forecast}>
        {forecast.map((item, index) => (
          <div key={`${item.date}-${index}`} className={styles.forecastItem}>
            <Typography.Text className={styles.forecastDay}>
              {formatForecastDate(item.date, index + 1)}
            </Typography.Text>
            <WeatherIcon type={item.weather} className={styles.forecastIcon} />
            <Typography.Text className={styles.forecastTemp}>
              {item.temperature}°
            </Typography.Text>
          </div>
        ))}
      </div>
    </Card>
  );
}
