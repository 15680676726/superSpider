...
async def _dispatch_agent_config_mutation(
    request: Request,
    *,
    capability_ref: str,
    title: str,
...
async def put_agents_running_config(
    ...
    result = await _dispatch_agent_config_mutation(
        request,
        capability_ref="system:update_agents_running_config",
        title="Update agent running config",
        payload={
    ...
    if not result.get("success"):
        raise HTTPException(400, detail=result.get("error") or "Running config update failed")
