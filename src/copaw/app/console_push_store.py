...
def _append_item(session_id: str, text: str) -> None:
    _list.append(
        {
            "id": str(uuid.uuid4()),
            "text": text,
...
async def append(session_id: str, text: str) -> None:
    ...
    if not session_id or not text:
        return
    async with _lock:
        _append_item(session_id, text)


def append_now(session_id: str, text: str) -> None:
    """Append a message from synchronous code paths."""
    if not session_id or not text:
        return
    _append_item(session_id, text)
