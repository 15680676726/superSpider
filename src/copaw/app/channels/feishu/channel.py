...
Emoji: Any = None
P2ImMessageReceiveV1: Any = None
_LARK_IMPORT_ATTEMPTED = False


def _load_lark_sdk() -> bool:
    """Load lark-oapi lazily so optional-channel imports stay quiet."""
    global lark
    global CreateImageRequest
    global CreateImageRequestBody
    global CreateMessageRequest
    global CreateMessageRequestBody
    global CreateMessageReactionRequest
    global CreateMessageReactionRequestBody
    global Emoji
    global P2ImMessageReceiveV1
    global _LARK_IMPORT_ATTEMPTED

    if lark is not None:
        return True
    if _LARK_IMPORT_ATTEMPTED:
        return False

    _LARK_IMPORT_ATTEMPTED = True
    pkg_resources_missing = object()
    original_pkg_resources: Any = sys.modules.get(
        "pkg_resources",
        pkg_resources_missing,
    )
    pkg_resources_shim: Optional[types.ModuleType] = None
    pkg_resources_module: Any = None
    declare_namespace_patched = False

    try:
        # Keep known lark-oapi import-time deprecation noise local to this
        # optional dependency instead of leaking into unrelated startup/tests.
        with warnings.catch_warnings():
            ...
            try:
                import pkg_resources as pkg_resources_module  # type: ignore
            except ImportError:  # pragma: no cover - setuptools>=82
                pkg_resources_shim = types.ModuleType("pkg_resources")
                pkg_resources_shim.declare_namespace = (  # type: ignore[attr-defined]
                    _declare_namespace_shim
                )
                sys.modules["pkg_resources"] = pkg_resources_shim
            else:
                if not hasattr(pkg_resources_module, "declare_namespace"):
                    setattr(
                        pkg_resources_module,
                        "declare_namespace",
                        _declare_namespace_shim,
                    )
                    declare_namespace_patched = True

            import lark_oapi as imported_lark
            from lark_oapi.api.im.v1 import (
                CreateImageRequest as imported_create_image_request,
            )
            from lark_oapi.api.im.v1 import (
                CreateImageRequestBody as imported_create_image_request_body,
            )
            from lark_oapi.api.im.v1 import (
                CreateMessageReactionRequest as imported_create_message_reaction_request,
            )
            from lark_oapi.api.im.v1 import (
                CreateMessageReactionRequestBody as imported_create_message_reaction_request_body,
            )
            from lark_oapi.api.im.v1 import (
                CreateMessageRequest as imported_create_message_request,
            )
            from lark_oapi.api.im.v1 import (
                CreateMessageRequestBody as imported_create_message_request_body,
            )
            from lark_oapi.api.im.v1 import Emoji as imported_emoji
            from lark_oapi.api.im.v1 import (
                P2ImMessageReceiveV1 as imported_message_receive,
            )
    except ImportError:  # pragma: no cover - optional dependency may be missing
        return False
    finally:
        if (
            pkg_resources_shim is not None
            and sys.modules.get("pkg_resources") is pkg_resources_shim
        ):
            if original_pkg_resources is pkg_resources_missing:
                del sys.modules["pkg_resources"]
            else:
                sys.modules["pkg_resources"] = original_pkg_resources
        if declare_namespace_patched and pkg_resources_module is not None:
            if (
                getattr(pkg_resources_module, "declare_namespace", None)
                is _declare_namespace_shim
            ):
                delattr(pkg_resources_module, "declare_namespace")

    lark = imported_lark
    CreateImageRequest = imported_create_image_request
    CreateImageRequestBody = imported_create_image_request_body
    CreateMessageRequest = imported_create_message_request
    CreateMessageRequestBody = imported_create_message_request_body
    CreateMessageReactionRequest = imported_create_message_reaction_request
    CreateMessageReactionRequestBody = (
        imported_create_message_reaction_request_body
    )
    Emoji = imported_emoji
    P2ImMessageReceiveV1 = imported_message_receive
    return True


def _require_lark_sdk() -> None:
    if _load_lark_sdk():
        return
    raise RuntimeError(
...
class FeishuChannel(BaseChannel):
    ...
    def _add_reaction_sync(self, message_id: str, emoji_type: str) -> None:
        if not self._client or not _load_lark_sdk():
            return
        try:
            req = (
