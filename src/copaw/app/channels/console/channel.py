...
Messages are sent to the agent via the standard direct-ingress
``/api/agent/process`` endpoint. This channel only handles the **output** side:
whenever a
...
def _console_print(text: str) -> None:
    stream = sys.stdout
    stream.write(_safe_console_text(text))
...
class ConsoleChannel(BaseChannel):
    ...
    Input is handled by the direct-ingress ``/api/agent/process`` endpoint; this
    ...
    def _print_parts(
        ...
        for p in parts:
            ...
            if t == ContentType.TEXT and getattr(p, "text", None):
                _console_print(f"{self.bot_prefix}{p.text}\n")
            elif t == ContentType.REFUSAL and getattr(p, "refusal", None):
                _console_print(f"{_RED}⚠ Refusal: {p.refusal}{_RESET}\n")
            elif t == ContentType.IMAGE and getattr(p, "image_url", None):
                _console_print(f"{_YELLOW}🖼  [Image: {p.image_url}]{_RESET}\n")
            elif t == ContentType.VIDEO and getattr(p, "video_url", None):
                _console_print(f"{_YELLOW}🎬 [Video: {p.video_url}]{_RESET}\n")
            elif t == ContentType.AUDIO and getattr(p, "data", None):
                _console_print(f"{_YELLOW}🔊 [Audio]{_RESET}\n")
                ...
                _console_print(f"{_YELLOW}📎 [File: {url}]{_RESET}\n")
        _console_print("\n")
