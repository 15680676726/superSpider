# -*- coding: utf-8 -*-
"""Write query-handler error dumps for runtime debugging."""
...
def _safe_json_serialize(obj: object) -> object:
    ...
    if isinstance(obj, dict):
        return {str(key): _safe_json_serialize(value) for key, value in obj.items()}
    return str(obj)
...
def write_query_error_dump(
    ...
    """Write traceback, request metadata, and agent state to a temp JSON file."""
    try:
        ...
        if agent is not None:
            try:
                ...
                agent_state = {"_serialize_error": str(state_error)}
        payload = {
            "trace": trace_str,
        ...
        try:
            ...
            try:
                ...
                pass
    except Exception as dump_error:  # pragma: no cover - defensive logging
        logger.warning("Failed to write query error dump: %s", dump_error)
        return None


__all__ = ["write_query_error_dump"]
