...
def _prefer_skillhub_search() -> bool:
    raw = os.environ.get("COPAW_PREFER_SKILLHUB", "").strip().lower()
    if not raw:
        return True
...
def _hydrate_clawhub_payload(
    ...
    *,
    ...
    """
    Convert legacy hub metadata responses into
...
def _legacy_search_hub_skills(query: str, limit: int = 20) -> list[HubSkillResult]:
    base = _hub_base_url()
        ...
        if github_spec is not None:
            ...
            if skillsmp_slug:
                ...
                if is_skillhub_url(bundle_url):
                    data, source_url = load_skillhub_bundle_from_url(bundle_url)
                else:
