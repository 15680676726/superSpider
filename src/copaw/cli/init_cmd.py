...
Spider Mesh runs in your own environment. It can connect to channels
(DingTalk, Feishu, QQ, Discord, iMessage, etc.) and run skills that read
files, run commands, and call external APIs. By default it is a
single-operator boundary: one trusted user. A malicious or confused prompt
can lead the agent to do unsafe things if tools are enabled.

If multiple people can message the same instance with tools enabled, they
share the same delegated authority (files, commands, secrets the agent can
use).

If you are not comfortable with access control and hardening, do not run
Spider Mesh with tools or expose it to untrusted users. Get help from someone
...
def _echo_security_warning_box() -> None:
    ...
    console.print(
        Panel(
            ...
            title="[bold]Security warning[/bold]",
            border_style="blue",
        ),
    )
...
- Легкая проверка при отсутствии активности более 8 часов
""",
}
...
def init_cmd(force: bool, use_defaults: bool, accept_security: bool) -> None:
    ...
    active_llm = provider_manager.get_active_model()
    ...
    if use_defaults:
        ...
        synced, skipped = CapabilityService().sync_skills_to_working_dir(force=False)
        if skipped:
            click.echo(
