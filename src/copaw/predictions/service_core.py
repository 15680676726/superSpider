...
class _PredictionServiceCoreMixin:
    ...
    def _service_payloads(
        ...
        *,
        ...
        for item in items:
            serialized = _payload_dict(item)
            if serialized:
                payload.append(serialized)
        return payload

    def _partition_donor_candidates(
        self,
        ...
        for item in payload:
            source_kind = (_string(item.get("candidate_source_kind")) or "").lower()
            ingestion_mode = (_string(item.get("ingestion_mode")) or "").lower()
    ...
    def _candidate_is_active(self, payload: Mapping[str, Any]) -> bool:
        status = (_string(payload.get("status")) or "").lower()
        stage = (_string(payload.get("lifecycle_stage")) or "").lower()
    ...
    def _build_portfolio_scope_breakdown(
        self,
        ...
        *,
        ...
        for item in candidates:
            if self._candidate_is_retired(item):
                continue
            target_scope = _string(item.get("target_scope")) or "global"
            target_role_id = _string(item.get("target_role_id")) or "*"
            target_seat_ref = _string(item.get("target_seat_ref")) or "*"
        ...
        for scope_key, items in sorted(groups.items()):
            donor_ids = {
                donor_id
                for donor_id in (_string(item.get("donor_id")) for item in items)
                if donor_id is not None
            }
            entry = {
                "scope_key": scope_key,
                "target_scope": _string(items[0].get("target_scope")) or "global",
                "target_role_id": _string(items[0].get("target_role_id")),
                "target_seat_ref": _string(items[0].get("target_seat_ref")),
                ...
                "source_kind_count": dict(
                    sorted(
                        Counter(
                            (_string(item.get("candidate_source_kind")) or "unknown")
                            for item in items
                        ).items(),
                    ),
                ),
    ...
    def _build_governed_portfolio_summary(self) -> dict[str, Any]:
        ...
        density_budget = max(
            1,
            _int_value(getattr(portfolio_service, "_density_budget", None)) or 3,
        )
        governed_candidates, fallback_candidates = self._partition_donor_candidates()
        governed_donor_ids = {
            donor_id
            for donor_id in (_string(item.get("donor_id")) for item in governed_candidates)
            if donor_id is not None
        }
        governed_source_profile_ids = {
            source_profile_id
            for source_profile_id in (
                _string(item.get("source_profile_id")) for item in governed_candidates
            )
        ...
        source_profiles = [
            ...
            if _string(item.get("source_profile_id")) in governed_source_profile_ids
        ]
        trust_records = [
            item
            for item in self._service_payloads(
            ...
            if _string(item.get("donor_id")) in governed_donor_ids
        ]
        ...
        candidate_by_id = {
            ...
            if (candidate_id := _string(item.get("candidate_id"))) is not None
        }
        active_donor_ids = {
            donor_id
            for item in governed_candidates
            if self._candidate_is_active(item)
            and (donor_id := _string(item.get("donor_id"))) is not None
        }
        candidate_donor_ids = {
            donor_id
            for item in governed_candidates
        ...
        for item in trials:
            candidate = candidate_by_id.get(_string(item.get("candidate_id")) or "")
            donor_id = _string((candidate or {}).get("donor_id"))
            if donor_id is not None:
        ...
        for item in decisions:
            ...
            if donor_id is None:
                continue
            decision_kind = (_string(item.get("decision_kind")) or "").lower()
        ...
        for item in trust_records:
            donor_id = _string(item.get("donor_id"))
            if donor_id is None:
                continue
            success_count = _int_value(item.get("trial_success_count")) or 0
            failure_count = _int_value(item.get("trial_failure_count")) or 0
            rollback_count = _int_value(item.get("rollback_count")) or 0
            retirement_count = _int_value(item.get("retirement_count")) or 0
            if (
        ...
        scope_breakdown, over_budget_scopes = self._build_portfolio_scope_breakdown(
            governed_candidates,
        ...
        return {
            ...
            "trusted_source_count": sum(
                ...
                if (_string(item.get("trust_posture")) or "").lower() == "trusted"
            ),
            "watchlist_source_count": sum(
                1
                for item in source_profiles
                if (_string(item.get("trust_posture")) or "").lower() == "watchlist"
            ),
    ...
    def _build_discovery_summary(self) -> dict[str, Any]:
        ...
        fallback_source_profile_ids = {
            source_profile_id
            for source_profile_id in (
                _string(item.get("source_profile_id")) for item in fallback_candidates
            )
            if source_profile_id is not None
        }
        ...
        fallback_source_profiles = [
            item
            for item in source_profiles
            if _string(item.get("source_profile_id")) in fallback_source_profile_ids
        ]
        ...
        status = "ready"
        if active_source_count <= 0 and governed_source_profiles:
            status = "degraded"
        ...
        return {
            ...
            "summary": (
                ...
                if status == "degraded"
                else "Governed donor discovery has not been initialized yet."
            ),
            ...
            "by_source_kind": dict(
                sorted(
                    Counter(
                        (_string(item.get("source_kind")) or "unknown")
                        for item in governed_source_profiles
                    ).items(),
                ),
            ),
            "trust_posture_count": dict(
                sorted(
                    Counter(
                        (_string(item.get("trust_posture")) or "unknown")
                        for item in governed_source_profiles
                    ).items(),
                ),
            ),
