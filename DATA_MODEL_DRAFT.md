...
- `Execution-Grade Host Twin / Full Host Digital Twin` -> derived runtime projection，不是第二真相源
...
- `Execution-Grade Host Twin / Full Host Digital Twin` 只能从统一 state/evidence 派生，不允许新建第二套 host twin 存储
...
当这些对象引入后，前端必须同步把以下对象升级为一等可见对象，而不是继续只围绕 `goal / task / schedule`：
