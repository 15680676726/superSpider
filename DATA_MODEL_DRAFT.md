...
- `ResearchSessionRecord.id / provider / industry_instance_id / work_context_id / owner_agent_id / supervisor_agent_id / trigger_source / goal / status / browser_session_id / round_count / link_depth_count / download_count / stable_findings / open_questions / final_report_id / failure_class / failure_summary / brief / completed_at / metadata / created_at / updated_at`
- `ResearchSessionRoundRecord.id / session_id / round_index / question / generated_prompt / response_excerpt / response_summary / raw_links / selected_links / downloaded_artifacts / new_findings / remaining_gaps / decision / evidence_ids / sources / metadata / created_at / updated_at`
...
- `2026-04-20` retrieval substrate 补充：当前已正式新增一层可重建检索对象，但它们不是新的 formal truth。phase-1 对象包括 `RetrievalQuery / RetrievalPlan / RetrievalHit / RetrievalRun / RepositoryIndexSnapshot / CodeSymbolRecord`；它们只属于 `src/copaw/retrieval/` 的 retrieval substrate/cache layer，可清空重建，不可取代 `ResearchSession* / EvidenceRecord / AgentReportRecord / writeback truth`
- 当前 canonical 接线固定为 `SourceCollectionFrontdoorService -> SourceCollectionService -> RetrievalFacade -> ResearchAdapterResult -> ResearchSession* / Evidence / writeback`；`route_collection_mode(...)` 继续只负责 `light/heavy`，新的 retrieval planner 只负责检索策略，不再混成同一层运行真相
- `/runtime-center/research` 现已允许继续投影 retrieval explanation：`intent / requested_sources / mode_sequence / coverage / selected_hits / dropped_hits / trace`；但这些 retrieval fields 只是 read-model explanation，不得反向降级 `brief / findings / sources / gaps / conflicts / writeback_truth` 这些 top-level formal projection
